// gdraw_d3d1x_shared.inl - author: Fabian Giesen - copyright 2012 RAD Game
// Tools
//
// This file implements the part of the Iggy graphics driver layer shared
// between D3D10 and 11 (which is most of it). It heavily depends on a bunch of
// typedefs, #defines and utility functions that need to be set up correctly for
// the D3D version being targeted. This is a bit ugly, but much easier to
// maintain than the original solution, where we just kept two almost identical
...
#define GDRAW_NO_STREAMING_MIPGEN  // This renderer doesn't use GDraw-internal
                                   // mipmap generation
...
#define STENCIL_STATE_CACHE_SIZE \
    32  // number of distinct stencil states we cache DepthStencilStates for
...
struct DynBuffer {
    ...
    U32 alloc_pos;  // end of most recently allocated chunk (=start of next
                    // allocation)
};
...
typedef struct {
    ...
    ID3D1X(SamplerState) *
        sampler_state[2][GDRAW_WRAP__count];  // [nearest][wrap]
...
static void* start_write_dyn(DynBuffer* buf, U32 size) {
    ...
    if (size > buf->size) {
        IggyGDrawSendWarning(NULL,
                             "GDraw dynamic vertex buffer usage of %d bytes in "
                             "one call larger than buffer size %d",
                             size, buf->size);
        return NULL;
    }
...
static ID3D1X(DepthStencilState) *
    stencil_state_cache_lookup(rrbool set_id, rrbool test_id, U8 read_mask,
                               U8 write_mask) {
    ...
    hr = gdraw->d3d_device->CreateDepthStencilState(
        &desc, &gdraw->stencil_cache[best]);
    if (FAILED(hr)) report_d3d_error(hr, "CreateDepthStencilState", "");
...
extern GDrawTexture* gdraw_D3D1X_(WrappedTextureCreate)(
    ID3D1X(ShaderResourceView) * tex_view) {
    GDrawStats stats = {0};
...
extern void gdraw_D3D1X_(WrappedTextureChange)(GDrawTexture* tex,
                                               ID3D1X(ShaderResourceView) *
                                                   tex_view) {
...
static rrbool RADLINK gdraw_MakeTextureBegin(
    ...
    if (width >= 16384 || height >= 16384) {
        IggyGDrawSendWarning(
            NULL,
            "GDraw texture size too large (%d x %d), dimension limit is 16384",
            width, height);
        return false;
    }
    ...
    do {
        ...
        ++nmips;
    } while ((flags & GDRAW_MAKETEXTURE_FLAGS_mipmap) &&
             ((width >> nmips) || (height >> nmips)));

    // try to allocate memory for the client to write to
    p->texture_data = (U8*)IggyGDrawMalloc(size);
    if (!p->texture_data) {
        IggyGDrawSendWarning(NULL,
                             "GDraw out of memory to store texture data to "
                             "pass to D3D for %d x %d texture",
                             width, height);
        return false;
    }
...
gdraw_MakeTextureEnd(GDraw_MakeTexture_ProcessingInfo* p, GDrawStats* stats) {
    ...
    for (i = 0; i < nmips; ++i) {
        ...
        if (i)
            gdraw_Downsample((U8*)mipdata[i].pSysMem, mipdata[i].SysMemPitch,
                             w >> i, h >> i, (U8*)mipdata[i - 1].pSysMem,
                             mipdata[i - 1].SysMemPitch, bpp);
    }

    // actually create texture
    D3D1X_(TEXTURE2D_DESC)
    desc = {w,
            h,
            nmips,
            1,
            (DXGI_FORMAT)p->i3,
            {1, 0},
            (p->i2 & GDRAW_MAKETEXTURE_FLAGS_updatable)
                ? D3D1X_(USAGE_DEFAULT)
                : D3D1X_(USAGE_IMMUTABLE),
            D3D1X_(BIND_SHADER_RESOURCE),
            0,
            0};
    ...
    hr = gdraw->d3d_device->CreateShaderResourceView(t->handle.tex.d3d, NULL,
    ...
    if (!FAILED(hr)) {
        gdraw_HandleCacheAllocateEnd(
            t, p->i4, p->p1,
            (p->i2 & GDRAW_MAKETEXTURE_FLAGS_never_flush)
...
static void RADLINK gdraw_UpdateTextureRect(GDrawTexture* t,
    ...
    gdraw->d3d_context->UpdateSubresource(s->handle.tex.d3d, 0, &box, samples,
                                          stride, 0);
}
...
static void RADLINK gdraw_SetAntialiasTexture(S32 width, U8* rgba) {
    ...
    hr = gdraw->d3d_device->CreateShaderResourceView(gdraw->aa_tex, NULL,
                                                     &gdraw->aa_tex_view);
    if (FAILED(hr)) {
        report_d3d_error(hr, "CreateShaderResourceView",
...
static rrbool RADLINK gdraw_MakeVertexBufferBegin(
    ...
    if (p->vertex_data && p->index_data) {
        GDrawHandle* vb = gdraw_res_alloc_begin(gdraw->vbufcache,
                                                vbuf_size + ibuf_size, stats);
        if (vb) {
...
static GDrawVertexBuffer* RADLINK gdraw_MakeVertexBufferEnd(
    ...
    HRESULT hr;
    D3D1X_(BUFFER_DESC)
    vbdesc = {p->vertex_data_length, D3D1X_(USAGE_IMMUTABLE),
              D3D1X_(BIND_VERTEX_BUFFER), 0, 0};
    D3D1X_(SUBRESOURCE_DATA) vbdata = {p->vertex_data, 0, 0};

    D3D1X_(BUFFER_DESC)
    ibdesc = {p->index_data_length, D3D1X_(USAGE_IMMUTABLE),
              D3D1X_(BIND_INDEX_BUFFER), 0, 0};
    D3D1X_(SUBRESOURCE_DATA) ibdata = {p->index_data, 0, 0};

    hr = gdraw->d3d_device->CreateBuffer(&vbdesc, &vbdata,
                                         &vb->handle.vbuf.verts);
    if (!FAILED(hr))
        hr = gdraw->d3d_device->CreateBuffer(&ibdesc, &ibdata,
                                             &vb->handle.vbuf.inds);
    ...
    if (FAILED(hr)) {
        ...
        gdraw_HandleCacheAllocateEnd(
            vb, p->vertex_data_length + p->index_data_length, p->p1,
            GDRAW_HANDLE_STATE_locked);
    }
...
static GDrawHandle* get_color_rendertarget(GDrawStats* stats) {
    ...
    desc = {gdraw->frametex_width,
            ...
            {1, 0},
            D3D1X_(USAGE_DEFAULT),
            D3D1X_(BIND_SHADER_RESOURCE) | D3D1X_(BIND_RENDER_TARGET),
            0,
            0};
    ...
    HRESULT hr =
        gdraw->d3d_device->CreateTexture2D(&desc, NULL, &t->handle.tex.d3d);
    failed_call = "CreateTexture2D";
    if (!FAILED(hr)) {
        hr = gdraw->d3d_device->CreateShaderResourceView(
            t->handle.tex.d3d, NULL, &t->handle.tex.d3d_view);
        failed_call = "CreateTexture2D";
    }
    if (!FAILED(hr)) {
        hr = gdraw->d3d_device->CreateRenderTargetView(
            t->handle.tex.d3d, NULL, &t->handle.tex.d3d_rtview);
...
static ID3D1X(DepthStencilView) *
    get_rendertarget_depthbuffer(GDrawStats* stats) {
    if (!gdraw->depth_buffer[1]) {
        ...
        HRESULT hr = gdraw->d3d_device->CreateTexture2D(
            &desc, NULL, &gdraw->rt_depth_buffer);
        failed_call = "CreateTexture2D";
        if (!FAILED(hr)) {
            hr = gdraw->d3d_device->CreateDepthStencilView(
                gdraw->rt_depth_buffer, NULL, &gdraw->depth_buffer[1]);
        ...
        if (FAILED(hr)) {
            ...
            gdraw->d3d_context->ClearDepthStencilView(
                gdraw->depth_buffer[1],
                D3D1X_(CLEAR_DEPTH) | D3D1X_(CLEAR_STENCIL), 1.0f, 0);
        }
    }
...
static void set_viewport_raw(S32 x, S32 y, S32 w, S32 h) {
    D3D1X_(VIEWPORT)
    vp = {(ViewCoord)x, (ViewCoord)y, (ViewCoord)w, (ViewCoord)h, 0.0f, 1.0f};
...
static void clear_renderstate(void) { gdraw->d3d_context->ClearState(); }
...
void gdraw_D3D1X_(SetTileOrigin)(ID3D1X(RenderTargetView) * main_rt,
                                 ID3D1X(DepthStencilView) * main_ds,
                                 ID3D1X(ShaderResourceView) * non_msaa_rt,
                                 S32 x, S32 y) {
    ...
    gdraw->main_msaa =
        (desc.ViewDimension == D3D1X_(RTV_DIMENSION_TEXTURE2DMS));
...
static void RADLINK gdraw_RenderTileBegin(S32 x0, S32 y0, S32 x1, S32 y1,
    ...
    assert(gdraw->tpw <= gdraw->frametex_width &&
           gdraw->tph <= gdraw->frametex_height);

    // set up rendertargets we'll use
    set_common_renderstate();
    gdraw->d3d_context->ClearDepthStencilView(
        gdraw->depth_buffer[0], D3D1X_(CLEAR_DEPTH) | D3D1X_(CLEAR_STENCIL),
        1.0f, 0);
    if (gdraw->depth_buffer[1])
        gdraw->d3d_context->ClearDepthStencilView(
            gdraw->depth_buffer[1], D3D1X_(CLEAR_DEPTH) | D3D1X_(CLEAR_STENCIL),
            1.0f, 0);
...
static void set_render_target(GDrawStats* stats) {
    ...
    if (target == gdraw->main_framebuffer) {
        gdraw->d3d_context->OMSetRenderTargets(1, &target,
                                               gdraw->depth_buffer[0]);
...
static rrbool RADLINK gdraw_TextureDrawBufferBegin(
    ...
    if (region) {
        ...
        if (r.x1 <= r.x0 ||
            r.y1 <= r.y0) {  // region doesn't intersect with current tile
            --gdraw->cur;
        ...
        gdraw->d3d_context->ClearRenderTargetView(
            gdraw->cur->color_buffer->handle.tex.d3d_rtview, four_zeros);
...
static void RADLINK gdraw_ClearStencilBits(U32 /*bits*/) {
    gdraw->d3d_context->ClearDepthStencilView(gdraw->depth_buffer[0],
                                              D3D1X_(CLEAR_STENCIL), 1.0f, 0);
    if (gdraw->depth_buffer[1])
        gdraw->d3d_context->ClearDepthStencilView(
            gdraw->depth_buffer[1], D3D1X_(CLEAR_STENCIL), 1.0f, 0);
}
...
static void RADLINK gdraw_ClearID(void) {
    gdraw->d3d_context->ClearDepthStencilView(gdraw->depth_buffer[0],
                                              D3D1X_(CLEAR_DEPTH), 1.0f, 0);
    if (gdraw->depth_buffer[1])
        gdraw->d3d_context->ClearDepthStencilView(gdraw->depth_buffer[1],
                                                  D3D1X_(CLEAR_DEPTH), 1.0f, 0);
}
...
static void set_texture(S32 texunit, GDrawTexture* tex, rrbool nearest,
                        S32 wrap) {
    ID3D1XContext* d3d = gdraw->d3d_context;
...
static int set_renderstate_full(S32 vertex_format, GDrawRenderState* r,
                                GDrawStats* /* stats */, const F32* rescale1) {
    ID3D1XContext* d3d = gdraw->d3d_context;
    ...
    if (VertexVars* vvars = (VertexVars*)map_buffer(gdraw->d3d_context,
                                                    gdraw->cb_vertex, true)) {
        F32 depth = depth_from_id(r->id);
        if (!r->use_world_space)
            gdraw_ObjectSpace(vvars->world[0], r->o2w, depth, 0.0f);
        else
            gdraw_WorldSpace(vvars->world[0], gdraw->world_to_pixel, depth,
                             0.0f);
    ...
    if (blend_mode != GDRAW_BLEND_special) {
        ...
        assert(which >= 0 &&
               which < sizeof(gdraw->fprog) / sizeof(*gdraw->fprog));
        ...
        if (r->stencil_set) {
            ...
            {
                gdraw->blend_mode = -1;
                d3d->OMSetBlendState(gdraw->blend_no_color_write, four_zeros,
                                     ~0u);
            }
        }
        ...
        set_pixel_shader(d3d,
                         gdraw->exceptional_blend[r->special_blend].pshader);

    set_texture(0, r->tex[0], r->nearest0, r->wrap0);

    // pixel shader constants
    if (PixelCommonVars* pvars = (PixelCommonVars*)map_buffer(
            gdraw->d3d_context, gdraw->cb_ps_common, true)) {
        ...
        if (r->cxf_add) {
            ...
            pvars->color_add[0] = pvars->color_add[1] = pvars->color_add[2] =
                pvars->color_add[3] = 0.0f;

        if (r->tex0_mode == GDRAW_TEXTURE_focal_gradient)
            memcpy(pvars->focal, r->focal_point, 4 * sizeof(float));
        if (r->blend_mode == GDRAW_BLEND_special)
            memcpy(pvars->rescale1, rescale1, 4 * sizeof(float));
    ...
    if (r->stencil_set | r->stencil_test)
        d3d->OMSetDepthStencilState(
            stencil_state_cache_lookup(r->set_id, r->test_id, r->stencil_test,
                                       r->stencil_set),
            255);
    else
        d3d->OMSetDepthStencilState(gdraw->depth_state[r->set_id][r->test_id],
                                    0);
...
static RADINLINE int set_renderstate(S32 vertex_format, GDrawRenderState* r,
    ...
    if (r->identical_state) {
        ...
        set_vertex_shader(gdraw->d3d_context,
                          gdraw->vert[vertex_format].vshader);
        return 1;
    } else
...
static D3D1X_(INPUT_ELEMENT_DESC) vformat_v2[] = {
    {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 0,
     D3D1X_(INPUT_PER_VERTEX_DATA), 0},
};
...
static void RADLINK gdraw_DrawIndexedTriangles(GDrawRenderState* r,
    ...
    if (vb) {
        ...
        d3d->IASetIndexBuffer(vb->handle.vbuf.inds, DXGI_FORMAT_R16_UINT,
                              (UINT)(UINTa)p->indices);
        ...
        if (void* vbptr = start_write_dyn(&gdraw->dyn_vb, vbytes)) {
            ...
            if (void* ibptr = start_write_dyn(&gdraw->dyn_ib, ibytes)) {
                ...
                d3d->IASetVertexBuffers(0, 1, &gdraw->dyn_vb.buffer, &stride,
                                        &vboffs);
                d3d->IASetIndexBuffer(gdraw->dyn_ib.buffer,
                                      DXGI_FORMAT_R16_UINT, iboffs);
                d3d->DrawIndexed(p->num_indices, 0, 0);
            }
        }
    } else {  // dynamic quads
        ...
        if (gdraw->max_quad_vert_count) {
            ...
            while (pos < p->num_vertices) {
                S32 vert_count =
                    RR_MIN(p->num_vertices - pos, gdraw->max_quad_vert_count);
                UINT chunk_bytes = vert_count * stride;

                if (void* vbptr =
                        start_write_dyn(&gdraw->dyn_vb, chunk_bytes)) {
                    memcpy(vbptr, (U8*)p->vertices + pos * stride, chunk_bytes);
                    UINT offs = end_write_dyn(&gdraw->dyn_vb);

                    d3d->IASetVertexBuffers(0, 1, &gdraw->dyn_vb.buffer,
                                            &stride, &offs);
...
static void do_screen_quad(gswf_recti* s, const F32* tc, GDrawStats* stats) {
    ...
    gswf_vertex_xyst* vert = (gswf_vertex_xyst*)start_write_dyn(
        &gdraw->dyn_vb, 4 * sizeof(gswf_vertex_xyst));
    if (!vert) return;
...
static void gdraw_Colormatrix(GDrawRenderState* r, gswf_recti* s, float* tc,
    ...
    PixelParaColorMatrix* para =
        (PixelParaColorMatrix*)start_ps_constants(gdraw->cb_colormatrix);
...
static void gdraw_Filter(GDrawRenderState* r, gswf_recti* s, float* tc,
    ...
    set_pixel_shader(gdraw->d3d_context,
                     gdraw->filter_prog[isbevel][r->filter_mode].pshader);

    PixelParaFilter* para =
        (PixelParaFilter*)start_ps_constants(gdraw->cb_filter);
    set_clamp_constant(para->clamp0, r->tex[0]);
    set_clamp_constant(para->clamp1, r->tex[1]);
    set_pixel_constant(para->color, r->shader_data[0], r->shader_data[1],
                       r->shader_data[2], r->shader_data[3]);
    set_pixel_constant(para->color2, r->shader_data[8], r->shader_data[9],
                       r->shader_data[10], r->shader_data[11]);
    set_pixel_constant(
        para->tc_off, -r->shader_data[4] / (F32)gdraw->frametex_width,
        -r->shader_data[5] / (F32)gdraw->frametex_height, r->shader_data[6], 0);
...
static void RADLINK gdraw_FilterQuad(GDrawRenderState* r, S32 x0, S32 y0,
    ...
    if (r->blend_mode == GDRAW_BLEND_filter) {
        ...
        // might not be a texture. note that this isn't optimal when MSAA is on!
        ...
        if (r->blend_mode == GDRAW_BLEND_special) {
            ...
            if (gdraw->cur == gdraw->frame && gdraw->main_msaa) {
                // source surface is main framebuffer and it uses MSAA. just
                // resolve it first.
                ...
                gdraw->main_resolve_target->GetResource(
                    (ID3D1X(Resource)**)&resolve_tex);
                resolve_tex->GetDesc(&texdesc);
                d3d->ResolveSubresource(resolve_tex, 0, cur_rt_rsrc, 0,
                                        desc.Format);
                resolve_tex->Release();
                ...
                d3d->PSSetSamplers(1, 1,
                                   &gdraw->sampler_state[0][GDRAW_WRAP_clamp]);
                ...
                rescale1[2] =
                    (gdraw->vx - gdraw->tx0 + gdraw->tx0p) / (F32)texdesc.Width;
                ...
                if (gdraw->cur != gdraw->frame)
                    ...
                    box.left = gdraw->vx, box.top = gdraw->vy,
                    box.right = gdraw->vx + gdraw->tw,
                    box.bottom = gdraw->vy + gdraw->th;
                    dx = gdraw->tx0 - gdraw->tx0p;
                    dy = gdraw->ty0 - gdraw->ty0p;
                }

                d3d->CopySubresourceRegion(blend_tex->handle.tex.d3d, 0, dx, dy,
                                           0, cur_rt_rsrc, 0, &box);
                ...
                set_texture(1, (GDrawTexture*)blend_tex, false,
                            GDRAW_WRAP_clamp);
            }
...
static ID3D1X(Buffer) * create_dynamic_buffer(U32 size, U32 bind) {
    D3D1X_(BUFFER_DESC)
    desc = {size, D3D1X_(USAGE_DYNAMIC), bind, D3D1X_(CPU_ACCESS_WRITE), 0};
...
// These two functions are implemented by the D3D10- respectively D3D11-specific
// part.
...
static void create_all_shaders_and_state(void) {
    ...
    for (i = 0; i < GDRAW_vformat__basic_count; i++) {
        ...
        HRESULT hr = d3d->CreateInputLayout(vformats[i].desc, vformats[i].nelem,
                                            vsh->bytecode, vsh->size,
                                            &gdraw->inlayout[i]);
        if (FAILED(hr)) {
    ...
    static const D3D1X_(TEXTURE_ADDRESS_MODE)
        addrmode[ASSERT_COUNT(GDRAW_WRAP__count, 4)] = {
            ...
            D3D1X_(TEXTURE_ADDRESS_CLAMP),   // GDRAW_WRAP_clamp_to_border
                                             // (unused for this renderer)
        };
    ...
    for (i = 0; i < 2; ++i) {
        for (j = 0; j < GDRAW_WRAP__count; ++j) {
            ...
            sampler_desc.Filter = i ? D3D1X_(FILTER_MIN_LINEAR_MAG_MIP_POINT)
                                    : D3D1X_(FILTER_MIN_MAG_MIP_LINEAR);
            ...
            hr = d3d->CreateSamplerState(&sampler_desc,
                                         &gdraw->sampler_state[i][j]);
            if (FAILED(hr)) {
    ...
    static struct blendspec {
        ...
        FALSE,
        D3D1X_(BLEND_ONE),
        D3D1X_(BLEND_ZERO),  // GDRAW_BLEND_none
        TRUE,
        D3D1X_(BLEND_ONE),
        D3D1X_(BLEND_INV_SRC_ALPHA),  // GDRAW_BLEND_alpha
        TRUE,
        D3D1X_(BLEND_DEST_COLOR),
        D3D1X_(BLEND_INV_SRC_ALPHA),  // GDRAW_BLEND_multiply
        TRUE,
        D3D1X_(BLEND_ONE),
        D3D1X_(BLEND_ONE),  // GDRAW_BLEND_add

        FALSE,
        D3D1X_(BLEND_ONE),
        D3D1X_(BLEND_ZERO),  // GDRAW_BLEND_filter
        FALSE,
        D3D1X_(BLEND_ONE),
        D3D1X_(BLEND_ZERO),  // GDRAW_BLEND_special
    };

    for (i = 0; i < GDRAW_BLEND__count; ++i) {
        gdraw->blend_state[i] = create_blend_state(
            d3d, blends[i].blend, blends[i].src, blends[i].dst);
        if (!gdraw->blend_state[i]) return;
    }
    ...
    for (i = 0; i < 2; ++i) {
        for (j = 0; j < 2; ++j) {
            ...
            depth_desc.DepthWriteMask = i ? D3D1X_(DEPTH_WRITE_MASK_ALL)
                                          : D3D1X_(DEPTH_WRITE_MASK_ZERO);
            depth_desc.DepthFunc =
                j ? D3D1X_(COMPARISON_LESS) : D3D1X_(COMPARISON_ALWAYS);
            depth_desc.StencilEnable = FALSE;

            hr = d3d->CreateDepthStencilState(&depth_desc,
                                              &gdraw->depth_state[i][j]);
            if (FAILED(hr)) {
    ...
    // constant buffers
    gdraw->cb_vertex =
        create_dynamic_buffer(sizeof(VertexVars), D3D1X_(BIND_CONSTANT_BUFFER));
    gdraw->cb_ps_common = create_dynamic_buffer(sizeof(PixelCommonVars),
                                                D3D1X_(BIND_CONSTANT_BUFFER));
    gdraw->cb_filter = create_dynamic_buffer(sizeof(PixelParaFilter),
                                             D3D1X_(BIND_CONSTANT_BUFFER));
    gdraw->cb_colormatrix = create_dynamic_buffer(sizeof(PixelParaColorMatrix),
                                                  D3D1X_(BIND_CONSTANT_BUFFER));
    gdraw->cb_blur = create_dynamic_buffer(sizeof(PixelParaBlur),
                                           D3D1X_(BIND_CONSTANT_BUFFER));

    // quad index buffer
    assert(QUAD_IB_COUNT * 4 <
           65535);  // can't use more; we have 16-bit index buffers and 0xffff =
                    // primitive cut index
...
static void destroy_all_shaders_and_state() {
    ...
    for (i = 0; i < GDRAW_WRAP__count * 2; ++i)
        safe_release(gdraw->sampler_state[0][i]);
    for (i = 0; i < GDRAW_BLEND__count; ++i)
        safe_release(gdraw->blend_state[i]);
...
static GDrawResourceLimit gdraw_limits[GDRAW_D3D1X_(RESOURCE__count)] = {
    ...
    16 * 1024 * 1024,  // RESOURCE_rendertarget
    500,
    16 * 1024 * 1024,  // RESOURCE_texture
    1000,
    2 * 1024 * 1024,  // RESOURCE_vertexbuffer
    0,
    256 * 1024,  // RESOURCE_dynbuffer
};
...
static bool alloc_dynbuffer(U32 size) {
    // specified input size is vertex buffer size. determine sensible size for
    // the corresponding index buffer. iggy always uses 16-bit indices and has
    // three primary types of geometry it sends:
    //
    ...
    //
    // 1) argues for allocating index buffers at 3/4 the size of the
    // corresponding vertex buffer, while 2) and 3) need 1/2 the size of the
    // vertex buffer or less. 2) and 3) are the most common types of vertex
    // data, while 1) is used only for morphed shapes and in certain cases when
    // the RESOURCE_vertexbuffer pool is full. we just play it safe anyway and
    // make sure we size the IB large enough to cover the worst case for 1).
    // this is conservative, but it probably doesn't matter much.
    U32 ibsize = (size * 3) / 4;

    init_dyn_buffer(&gdraw->dyn_vb, size, D3D1X_(BIND_VERTEX_BUFFER));
    init_dyn_buffer(&gdraw->dyn_ib, ibsize, D3D1X_(BIND_INDEX_BUFFER));

    gdraw->max_quad_vert_count =
        RR_MIN(size / sizeof(gswf_vertex_xyst), QUAD_IB_COUNT * 4);
    gdraw->max_quad_vert_count &= ~3;  // must be multiple of four

    return gdraw->dyn_vb.buffer != NULL && gdraw->dyn_ib.buffer != NULL;
}

int gdraw_D3D1X_(SetResourceLimits)(gdraw_resourcetype type, S32 num_handles,
                                    S32 num_bytes) {
    GDrawStats stats = {0};

    if (type == GDRAW_D3D1X_(RESOURCE_rendertarget))  // RT count is small and
                                                      // space is preallocated
        num_handles = MAX_RENDER_STACK_DEPTH + 1;

    assert(type >= GDRAW_D3D1X_(RESOURCE_rendertarget) &&
           type < GDRAW_D3D1X_(RESOURCE__count));
    ...
    switch (type) {
        ...
        case GDRAW_D3D1X_(RESOURCE_texture):
            ...
            gdraw->texturecache =
                make_handle_cache(GDRAW_D3D1X_(RESOURCE_texture));
        ...
        case GDRAW_D3D1X_(RESOURCE_vertexbuffer):
            ...
            gdraw->vbufcache =
                make_handle_cache(GDRAW_D3D1X_(RESOURCE_vertexbuffer));
...
static GDrawFunctions* create_context(ID3D1XDevice* dev, ID3D1XContext* ctx,
                                      S32 w, S32 h) {
    ...
    gdraw_HandleCacheInit(
        &gdraw->rendertargets,
        gdraw_limits[GDRAW_D3D1X_(RESOURCE_rendertarget)].num_handles,
        gdraw_limits[GDRAW_D3D1X_(RESOURCE_rendertarget)].num_bytes);

    if (!gdraw->texturecache || !gdraw->vbufcache ||
        !alloc_dynbuffer(
            gdraw_limits[GDRAW_D3D1X_(RESOURCE_dynbuffer)].num_bytes)) {
        free_gdraw();
        return NULL;
    }
    ...
    gdraw_funcs.MakeTextureFromResource =
        (gdraw_make_texture_from_resource*)gdraw_D3D1X_(
            MakeTextureFromResource);
    gdraw_funcs.FreeTextureFromResource =
        gdraw_D3D1X_(DestroyTextureFromResource);
...
void RADLINK gdraw_D3D1X_(BeginCustomDraw)(IggyCustomDrawCallbackRegion* region,
                                           F32 mat[4][4]) {
    clear_renderstate();
    gdraw_GetObjectSpaceMatrix(mat[0], region->o2w, gdraw->projection, 0, 0);
}

void RADLINK gdraw_D3D1X_(BeginCustomDraw_4J)(
    IggyCustomDrawCallbackRegion* region, F32 mat[16]) {
    clear_renderstate();
    gdraw_GetObjectSpaceMatrix(mat, region->o2w, gdraw->projection, 0, 0);
}

void RADLINK gdraw_D3D1X_(CalculateCustomDraw_4J)(
    IggyCustomDrawCallbackRegion* region, F32 mat[16]) {
    gdraw_GetObjectSpaceMatrix(mat, region->o2w, gdraw->projection, 0, 0);
}

void RADLINK
gdraw_D3D1X_(EndCustomDraw)(IggyCustomDrawCallbackRegion* /*region*/) {
    GDrawStats stats = {};
    set_common_renderstate();
    set_viewport();
    set_render_target(&stats);
...
void RADLINK gdraw_D3D1X_(GetResourceUsageStats)(gdraw_resourcetype type,
                                                 S32* handles_used,
                                                 S32* bytes_used) {
    ...
    switch (type) {
        ...
        case GDRAW_D3D1X_(RESOURCE_dynbuffer):
            *handles_used = 0;
            *bytes_used = gdraw->last_dyn_maxalloc;
            return;
        default:
            cache = NULL;
            break;
...
GDrawTexture* RADLINK gdraw_D3D1X_(MakeTextureFromResource)(
    ...
    switch (texture->format) {
        ...
        case IFT_FORMAT_DXT1:
            size = 8;
            d3dfmt = DXGI_FORMAT_BC1_UNORM;
            blk = 4;
            break;
        case IFT_FORMAT_DXT3:
            size = 16;
            d3dfmt = DXGI_FORMAT_BC2_UNORM;
            blk = 4;
            break;
        case IFT_FORMAT_DXT5:
            size = 16;
            d3dfmt = DXGI_FORMAT_BC3_UNORM;
            blk = 4;
            break;
        default: {
            IggyGDrawSendWarning(NULL,
                                 "GDraw .iggytex raw texture format %d not "
    ...
    if (texture->format == IFT_FORMAT_i_8 ||
        texture->format == IFT_FORMAT_i_4) {
    ...
    if (free_data) IggyGDrawFree(free_data);
