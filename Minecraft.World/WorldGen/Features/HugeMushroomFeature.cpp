...
bool HugeMushroomFeature::place(Level *level, Random *random, int x, int y, int z)
{
    ...
    for (int yy = low; yy <= y + treeHeight; yy++)
	{
        ...
        for (int xx = x - offs; xx <= x + offs; xx++)
		{
            for (int zz = z - offs; zz <= z + offs; zz++)
			{
				...
				{
                    if (!Tile::solid[level->getTile(xx, yy, zz)]) placeBlock(level, xx, yy, zz, Tile::hugeMushroom1_Id + type, data);
                }
            }
        }
    }
    for (int hh = 0; hh < treeHeight; hh++)
	{
        ...
        if (!Tile::solid[t]) placeBlock(level, x, y + hh, z, Tile::hugeMushroom1_Id + type, 10);
    }
    return true;
}
