...
const std::wstring NetherStalkTile::TEXTURE_NAMES[] = {
    L"netherStalk_0", L"netherStalk_1", L"netherStalk_2"};
...
bool NetherStalkTile::mayPlaceOn(int tile) { return tile == Tile::hellSand_Id; }
...
int NetherStalkTile::getRenderShape() { return Tile::SHAPE_ROWS; }

void NetherStalkTile::spawnResources(Level* level, int x, int y, int z,
    ...
    for (int i = 0; i < count; i++) {
        popResource(level, x, y, z,
                    std::shared_ptr<ItemInstance>(
                        new ItemInstance(Item::netherStalkSeeds)));
    }
}
...
int NetherStalkTile::getResourceCount(Random* random) { return 0; }
