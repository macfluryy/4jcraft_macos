...
bool RailTile::isRail(Level* level, int x, int y, int z) {
    ...
    return tile == Tile::rail_Id || tile == Tile::goldenRail_Id ||
           tile == Tile::detectorRail_Id;
}

bool RailTile::isRail(int id) {
    return id == Tile::rail_Id || id == Tile::goldenRail_Id ||
           id == Tile::detectorRail_Id;
}

// 4jcraft, changed from isSolidRender() to false, _init was changed by
// 4jstudios to take in a bool to avoid calling isSolidRender before the vptr is
// set,
RailTile::RailTile(int id, bool usesDataBit)
...
bool RailTile::isUsesDataBit() { return usesDataBit; }

AABB* RailTile::getAABB(Level* level, int x, int y, int z) { return NULL; }

bool RailTile::blocksLight() { return false; }

bool RailTile::isSolidRender(bool isServerLevel) { return false; }

HitResult* RailTile::clip(Level* level, int xt, int yt, int zt, Vec3* a,
                          Vec3* b) {
    updateShape(level, xt, yt, zt);
    return Tile::clip(level, xt, yt, zt, a, b);
}

void RailTile::updateShape(
...
bool RailTile::isCubeShaped() { return false; }

int RailTile::getRenderShape() { return Tile::SHAPE_RAIL; }

int RailTile::getResourceCount(Random random) { return 1; }
...
void RailTile::neighborChanged(Level* level, int x, int y, int z, int type) {
    ...
    if (remove) {
        ...
        if (id == Tile::goldenRail_Id) {
            ...
            signal = signal ||
                     findGoldenRailSignal(level, x, y, z, data, true, 0) ||
                     findGoldenRailSignal(level, x, y, z, data, false, 0);
...
bool RailTile::findGoldenRailSignal(Level* level, int x, int y, int z, int data,
    ...
    if (checkBelow &&
        isGoldenRailWithPower(level, x, y - 1, z, forward, searchDepth, dir)) {
        return true;
    }
    return false;
}

bool RailTile::isGoldenRailWithPower(Level* level, int x, int y, int z,
    ...
    if (tile == Tile::goldenRail_Id) {
        ...
        if ((tileData & RAIL_DATA_BIT) != 0) {
            if (level->hasNeighborSignal(x, y, z)) {
                ...
                return findGoldenRailSignal(level, x, y, z, tileData, forward,
                                            searchDepth + 1);
            }
        }
    }
...
int RailTile::getPistonPushReaction() { return Material::PUSH_NORMAL; }
