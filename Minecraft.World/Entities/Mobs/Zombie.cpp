...
Zombie::Zombie(Level* level) : Monster(level) {
    ...
    goalSelector.addGoal(
        2, new MeleeAttackGoal(this, eTYPE_PLAYER, runSpeed, false));
    goalSelector.addGoal(
        3, new MeleeAttackGoal(this, eTYPE_VILLAGER, runSpeed, true));
    ...
    targetSelector.addGoal(2, new NearestAttackableTargetGoal(
                                  this, typeid(Villager), 16, 0, false));
}
...
int Zombie::getMaxHealth() { return 20; }

int Zombie::getArmorValue() { return 2; }
...
void Zombie::killed(std::shared_ptr<Mob> mob) {
    ...
    if (level->difficulty >= Difficulty::NORMAL &&
        ((mob->GetType() & eTYPE_VILLAGER) == eTYPE_VILLAGER)) {
...
void Zombie::updateSize(bool isBaby) { internalSetSize(isBaby ? .5f : 1.0f); }
