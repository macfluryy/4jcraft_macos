    ...
    defined(__PSVITA__)
...
#ifdef __PS3__
        L10nResult lres =
            UTF8stoUTF16s((std::uint8_t*)params->saveDetails->UTF8SaveFilename,
...
#else  // __ORBIS__
        {
            ...
            sceCesUtf8StrToUtf16Str(
                &Context, (std::uint8_t*)params->saveDetails->UTF8SaveFilename,
        ...
        if (params->saveDetails->pbThumbnailData) {
            ...
            C4JStorage::ESaveGameState eLoadStatus =
                StorageManager.LoadSaveDataThumbnail(
                    ...
                    &LoadMenuThumbnailReturnedThunk, this, true);
#else
            C4JStorage::ESaveGameState eLoadStatus =
                StorageManager.LoadSaveDataThumbnail(
                    &pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],
                    &LoadMenuThumbnailReturnedThunk, this);
#endif
            m_bShowTimer = true;
        }
#if defined(_DURANGO)
    ...
    switch (sceNpCommerceDialogUpdateStatus()) {
        case SCE_COMMON_DIALOG_STATUS_FINISHED: {
            ...
            if (Result.authorized) {
                ...
                ui.RequestMessageBox(IDS_PLAY_OFFLINE, IDS_NO_PLAYSTATIONPLUS,
                                     uiIDA, 1, ProfileManager.GetPrimaryPad(),
                                     &UIScene_LoadMenu::ContinueOffline, this,
...
void UIScene_LoadMenu::StartSharedLaunchFlow() {
    ...
    if (m_MoreOptionsParams.bResetNether) {
        ...
        ui.RequestMessageBox(IDS_RESETNETHER_TITLE, IDS_RESETNETHER_TEXT, uiIDA,
                             2, m_iPad,
                ...
                if (m_iConfigA[i] != -1) {
                    ...
                    if (pDLCInfo) {
                        ...
                        std::wstring textureName =
                            filenametostd::wstring(pDLCInfo->chImageURL);
...
void UIScene_LoadMenu::LaunchGame(void) {
    ...
    if ((m_bGameModeSurvival != true || m_bHasBeenInCreative) ||
        ...
        if (m_bGameModeSurvival != true || m_bHasBeenInCreative) {
            ...
            ui.RequestMessageBox(IDS_TITLE_START_GAME,
                                 IDS_CONFIRM_START_HOST_PRIVILEGES, uiIDA, 2,
                                 m_iPad, &UIScene_LoadMenu::ConfirmLoadReturned,
                                 this, app.GetStringTable(), NULL, 0, false);
        }
    } else {
        if (m_levelGen != NULL) {
...
int UIScene_LoadMenu::LoadDataComplete(void* pParam) {
    ...
    if (!pClass->m_bIsCorrupt) {
        ...
        ui.RequestMessageBox(IDS_CORRUPT_OR_DAMAGED_SAVE_TITLE,
                             IDS_CORRUPT_OR_DAMAGED_SAVE_TEXT, uiIDA, 2,
                             pClass->m_iPad,
                             &UIScene_LoadMenu::DeleteSaveDialogReturned,
                             pClass, app.GetStringTable(), NULL, 0, false);
    }
...
int UIScene_LoadMenu::LoadSaveDataReturned(void* pParam, bool bIsCorrupt,
    ...
    if (bIsOwner) {
        ...
        unsigned int uiIDA[1];
        uiIDA[0] = IDS_CONFIRM_OK;
        ui.RequestMessageBox(IDS_SAVEDATA_COPIED_TITLE,
                             IDS_SAVEDATA_COPIED_TEXT, uiIDA, 1, pClass->m_iPad,
                             &UIScene_LoadMenu::TrophyDialogReturned, pClass,
                             app.GetStringTable());
#endif
    }
...
void UIScene_LoadMenu::LoadLevelGen(LevelGenerationOptions* levelGen) {
    ...
    if (!isClientSide || connectedControllers == 1 ||
        ...
        if (isClientSide && noUGC) {
            ...
            unsigned int uiIDA[1];
            uiIDA[0] = IDS_CONFIRM_OK;
            ui.RequestMessageBox(IDS_FAILED_TO_CREATE_GAME_TITLE,
