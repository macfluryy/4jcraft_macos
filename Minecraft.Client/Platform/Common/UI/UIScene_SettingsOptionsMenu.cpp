...
UIScene_SettingsOptionsMenu::UIScene_SettingsOptionsMenu(int iPad,
    ...
    if (m_bNotInGame && app.GetMashupPackWorlds(m_iPad) != 0xFFFFFFFF) {
        ...
        m_checkboxMashupWorlds.init(app.GetString(IDS_UNHIDE_MASHUP_WORLDS),
    ...
    switch (key) {
        case ACTION_MENU_CANCEL:
            if (pressed) {
                ...
                app.SetGameSettings(
                    m_iPad, eGameSetting_GamertagsVisible,
                ...
                if (m_bMashUpWorldsUnhideOption &&
                    m_checkboxMashupWorlds.IsChecked()) {
                    // unhide all worlds
                    app.EnableMashupPackWorlds(m_iPad);
                }

                // 4J-PB - don't action changes here or we might write to the
                // profile on backing out here and then get a change in the
                // settings all, and write again on backing out there
