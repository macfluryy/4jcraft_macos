                        ...
                        if (app.GetPlayerFavoriteSkinsCount(iPad) > 0) {
                            ...
                            if (Pack) {
                                DLCSkinFile* skinFile =
                                    Pack->getSkinFile(chars);
                                app.SetPlayerSkin(iPad, skinFile->getPath());
                                app.SetPlayerCape(
                                    iPad, skinFile->getParameterAsString(
                                              DLCManager::e_DLCParamType_Cape));
                                setCharacterSelected(true);
                                m_currentSkinPath = app.GetPlayerSkinName(iPad);
                                m_originalSkinId = app.GetPlayerSkinId(iPad);
                                app.SetPlayerFavoriteSkinsPos(iPad,
                                                              m_skinIndex);
                            }
                        }
                        break;
                    default:
                        if (m_currentPack != NULL) {
                            ...
                            if (!skinFile->getParameterAsBool(
                                    DLCManager::e_DLCParamType_Free)  // Is this
                                                                      // a free
                                                                      // skin?
                                &&
                                !m_currentPack->hasPurchasedFile(
                                    DLCManager::e_DLCType_Skin,
                                    skinFile
                                        ->getPath())  // do we have a license?
                            ) {
                                // 4J-PB - check for a patch
#ifdef __ORBIS__
                                // 4J-PB - Check if there is a patch for the
                                // game
                                int errorCode =
      ...
      defined(          \
          _WIN64))  // 4J Stu - Temp to get the win build running, but so we
                    // check this for other platforms you can't see the store
                                            ...
                                            // 4J-PB - need to check for an
                                            // empty store
#if defined __ORBIS__ || defined __PSVITA__ || defined __PS3__
                                            if (app.CheckForEmptyStore(iPad) ==
                                                false)
#endif
                                            {
                ...
                if (!skinFile->getParameterAsBool(
                    ...
                    if (!m_currentPack->hasPurchasedFile(
                        ...
                        if (ProfileManager.IsGuest(iPad)) {
                            ...
                            ui.RequestMessageBox(
                                IDS_PRO_GUESTPROFILE_TITLE,
                                IDS_PRO_GUESTPROFILE_TEXT, uiIDA, 1, iPad, NULL,
...
void UIScene_SkinSelectMenu::showNotOnlineDialog(int iPad) {
    ...
    ui.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE, IDS_PRO_XBOXLIVE_NOTIFICATION,
