...
void SoundEngine::init(Options* pOptions) {
	...
	memset(CurrentSoundsPlaying, 0, sizeof(int) * (static_cast<int>(eSoundType_MAX) + static_cast<int>(eSFX_MAX)));
		...
		if( !m_openStreamThread->isRunning() )
		{
			...
			if (!m_musicStreamActive)
			{
				...
				if (currentExt && _stricmp(currentExt, ".wav") == 0)
				{
					...
					if (n > 0)
					{
						FILE* pFile = fopen(reinterpret_cast<char*>(m_szStreamName), "rb");
						if (pFile)
						{
							fclose(pFile);
