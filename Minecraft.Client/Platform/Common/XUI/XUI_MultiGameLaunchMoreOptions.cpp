...
//----------------------------------------------------------------------------------
HRESULT CScene_MultiGameLaunchMoreOptions::OnInit(XUIMessageInit* pInitData,
                                                  BOOL& bHandled) {
    MapChildControls();
    ...
    XuiControlSetText(m_CheckboxAllowFoF,
                      app.GetString(IDS_ALLOWFRIENDSOFFRIENDS));
    ...
    XuiControlSetText(m_CheckboxHostPrivileges,
                      app.GetString(IDS_HOST_PRIVILEGES));
    ...
    XuiControlSetText(m_CheckboxStructures,
                      app.GetString(IDS_GENERATE_STRUCTURES));
    ...
    // SentientManager.RecordMenuShown(m_params->iPad, eUIScene_CreateWorldMenu,
    // 0);

    return S_OK;
}

HRESULT CScene_MultiGameLaunchMoreOptions::OnKeyDown(
    XUIMessageInput* pInputData, BOOL& rfHandled) {
...
HRESULT CScene_MultiGameLaunchMoreOptions::OnNotifySetFocus(
...
HRESULT CScene_MultiGameLaunchMoreOptions::OnControlNavigate(
...
//----------------------------------------------------------------------------------
HRESULT CScene_MultiGameLaunchMoreOptions::OnNotifyPressEx(
...
HRESULT CScene_MultiGameLaunchMoreOptions::OnTimer(XUIMessageTimer* pTimer,
                                                   BOOL& bHandled) {
        ...
        if (bMultiplayerAllowed != m_bMultiplayerAllowed) {
            if (bMultiplayerAllowed) {
                bool bGameSetting_Online =
                    (app.GetGameSettings(m_params->iPad, eGameSetting_Online) !=
                     0);
                m_CheckboxOnline.SetCheck(bGameSetting_Online ? TRUE : FALSE);
                if (bGameSetting_Online) {
                    m_CheckboxInviteOnly.SetCheck(
                        (app.GetGameSettings(m_params->iPad,
                                             eGameSetting_InviteOnly) != 0)
                            ? TRUE
                            : FALSE);
                    m_CheckboxAllowFoF.SetCheck(
                        (app.GetGameSettings(m_params->iPad,
