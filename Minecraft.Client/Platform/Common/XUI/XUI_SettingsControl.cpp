...
#include "XUI_SettingsControl.h"

//----------------------------------------------------------------------------------
// Performs initialization tasks - retrieves controls.
//----------------------------------------------------------------------------------
HRESULT CScene_SettingsControl::OnInit(XUIMessageInit* pInitData,
                                       BOOL& bHandled) {
    ...
    m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INGAME].SetValue(
    ...
    m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INMENU].SetValue(
...
HRESULT CScene_SettingsControl::OnNotifyValueChanged(
    HXUIOBJ hObjSource, XUINotifyValueChanged* pNotifyValueChanged,
    BOOL& bHandled) {
    WCHAR TempString[256];

    if (hObjSource ==
        m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INGAME].GetSlider()) {
        app.SetGameSettings(m_iPad, eGameSetting_Sensitivity_InGame,
        ...
        m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INGAME].SetText(TempString);
               ...
               m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INMENU].GetSlider()) {
        app.SetGameSettings(m_iPad, eGameSetting_Sensitivity_InMenu,
                            pNotifyValueChanged->nValue);
        swprintf((WCHAR*)TempString, 256, L"%ls: %d%%",
                 app.GetString(IDS_SLIDER_SENSITIVITY_INMENU),
                 pNotifyValueChanged->nValue);
...
HRESULT CScene_SettingsControl::OnKeyDown(XUIMessageInput* pInputData,
                                          BOOL& rfHandled) {
...
//----------------------------------------------------------------------------------
HRESULT CScene_SettingsControl::OnNotifyPressEx(
...
HRESULT CScene_SettingsControl::OnControlNavigate(
...
HRESULT CScene_SettingsControl::OnTransitionStart(
    ...
    if (pTransition->dwTransType == XUI_TRANSITION_TO ||
        ...
        hr = XuiElementGetChildById(
            m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INGAME].m_hObj, L"XuiSlider",
            &hSlider);
        hr = XuiControlGetVisual(hSlider, &hVisual);
        hr = XuiElementGetChildById(hVisual, L"text_Label", &hText);
        hr = XuiElementGetPosition(
            m_SliderA[SLIDER_SETTINGS_SENSITIVITY_INGAME].m_hObj, &vSlider);

        for (int i = 0; i < SLIDER_SETTINGS_CONTROL_MAX; i++) {
            switch (i) {
                case SLIDER_SETTINGS_SENSITIVITY_INGAME:  // 3 digits
                    swprintf((WCHAR*)TempString, 256, L"%ls: %d%%",
                             app.GetString(IDS_SLIDER_SENSITIVITY_INGAME), 999);
                    break;
                case SLIDER_SETTINGS_SENSITIVITY_INMENU:  // 3 digits
                    swprintf((WCHAR*)TempString, 256, L"%ls: %d%%",
                             app.GetString(IDS_SLIDER_SENSITIVITY_INMENU), 999);
                    break;
            }
...
HRESULT CScene_SettingsControl::OnCustomMessage_Splitscreenplayer(
