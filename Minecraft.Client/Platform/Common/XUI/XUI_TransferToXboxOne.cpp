...
//----------------------------------------------------------------------------------
HRESULT CScene_TransferToXboxOne::OnInit(XUIMessageInit* pInitData,
                                         BOOL& bHandled) {
    m_iX = -1;
    ...
    if (StorageManager.TMSPP_InFileList(C4JStorage::eGlobalStorage_TitleUser,
                                        m_iPad, L"XboxOne/SlotList")) {
        // there is a slot list file with details of the saves
        C4JStorage::ETMSStatus status = StorageManager.TMSPP_ReadFile(
            m_iPad, C4JStorage::eGlobalStorage_TitleUser,
            C4JStorage::TMS_FILETYPE_BINARY, "XboxOne/SlotList",
            &CScene_TransferToXboxOne::TMSPPSlotListReturned, this);
        ...
        for (int i = 0; i < m_MaxSlotC; i++) {
            memcpy(m_pSlotDataA[i].wchSaveTitle,
                   app.GetString(IDS_XBONE_EMPTYSLOT),
    ...
    CXuiSceneBase::SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                               IDS_TOOLTIPS_BACK, m_iX, -1, -1, -1, -1, -1, -1,
                               true);
...
//----------------------------------------------------------------------------------
int CScene_TransferToXboxOne::TMSPPWriteReturned(LPVOID pParam, int iPad,
                                                 int iUserData) {
...
// TMSPPSlotListReturned callback
//----------------------------------------------------------------------------------
int CScene_TransferToXboxOne::TMSPPDeleteReturned(LPVOID pParam, int iPad,
                                                  int iUserData) {
    CScene_TransferToXboxOne* pClass = (CScene_TransferToXboxOne*)pParam;
    ...
    for (int i = 0; i < pClass->m_MaxSlotC; i++) {
        memcpy(pClass->m_pSlotDataA[i].wchSaveTitle,
               app.GetString(IDS_XBONE_EMPTYSLOT),
               sizeof(WCHAR) * XCONTENT_MAX_DISPLAYNAME_LENGTH);
        ListInfo.pwszText = app.GetString(IDS_XBONE_EMPTYSLOT);
        ListInfo.fEnabled = TRUE;
...
// TMSPPSlotListReturned callback
//----------------------------------------------------------------------------------
int CScene_TransferToXboxOne::TMSPPSlotListReturned(
    LPVOID pParam, int iPad, int iUserData,
    C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename) {
    CScene_TransferToXboxOne* pClass = (CScene_TransferToXboxOne*)pParam;
    ...
    unsigned int uiNewSlotsC = (pClass->m_MaxSlotC < uiSlotListFileSlots)
                                   ? pClass->m_MaxSlotC
                                   : uiSlotListFileSlots;
    memcpy(pClass->m_pSlotDataA,
           pClass->m_pbSlotListFile + sizeof(unsigned int),
           sizeof(SLOTDATA) * uiNewSlotsC);

    CXuiCtrl4JList::LIST_ITEM_INFO ListInfo;

    ZeroMemory(&ListInfo, sizeof(CXuiCtrl4JList::LIST_ITEM_INFO));
    PBYTE pbImageData = pClass->m_pbSlotListFile + sizeof(unsigned int) +
                        sizeof(SLOTDATA) * uiSlotListFileSlots;
    ...
    for (int i = 0; i < pClass->m_MaxSlotC; i++) {
        if (i < uiNewSlotsC) {
            ...
            if (pClass->m_pSlotDataA[i].uiImageLength != 0) {
                XuiCreateTextureBrushFromMemory(
...
//----------------------------------------------------------------------------------
HRESULT CScene_TransferToXboxOne::OnDestroy() { return S_OK; }

//----------------------------------------------------------------------------------
// Handler for the button press message.
//----------------------------------------------------------------------------------
HRESULT CScene_TransferToXboxOne::OnNotifyPressEx(
    ...
    if (hObjPressed == m_SavesSlotList) {
        ...
        // update the data
        memcpy(m_pSlotDataA[iIndex].wchSaveTitle, m_XContentData.szDisplayName,
               sizeof(WCHAR) * XCONTENT_MAX_DISPLAYNAME_LENGTH);
        ...
        XuiCreateTextureBrushFromMemory(m_pbImageData, m_dwImageBytes,
                                        &m_hXuiBrush);
...
HRESULT CScene_TransferToXboxOne::BuildSlotFile(int iIndexBeingUpdated,
                                                PBYTE pbImageData,
                                                DWORD dwImageBytes) {
    ...
    if (m_pbSlotListFile != NULL) {
        ...
        pbCurrentSlotDataPtr = m_pbSlotListFile + sizeof(unsigned int) +
                               sizeof(SLOTDATA) * m_MaxSlotC;
    }

    m_uiSlotID = iIndexBeingUpdated;

    // memory required - first int is the number of slots in this file, in case
    // that changes later
    unsigned int uiBytesRequired =
        sizeof(unsigned int) + sizeof(SLOTDATA) * m_MaxSlotC;
    for (int i = 0; i < m_MaxSlotC; i++) {
    ...
    *((unsigned int*)pbNewSlotFileData) = m_MaxSlotC;
    ...
    PBYTE pbNewSlotImageDataPtr = pbNewSlotFileData + sizeof(unsigned int) +
                                  sizeof(SLOTDATA) * m_MaxSlotC;
    ...
    for (int i = 0; i < m_MaxSlotC; i++) {
        if (i == iIndexBeingUpdated) {
            ...
            if (pbCurrentSlotDataPtr != NULL) {
                memcpy(pbNewSlotImageDataPtr, pbCurrentSlotDataPtr,
                       pCurrentSlotData[i].uiImageLength);
    ...
    loadingParams->func =
        &CScene_TransferToXboxOne::UploadSaveForXboxOneThreadProc;
...
int CScene_TransferToXboxOne::UploadSaveForXboxOneThreadProc(
    LPVOID lpParameter) {
    HRESULT hr = S_OK;
    char szFilename[32];
    ...
    // now write the new slot data file to global storage, and then write the
    // save data
    C4JStorage::ETMSStatus eStatus = StorageManager.TMSPP_WriteFile(
        pClass->m_iPad, C4JStorage::eGlobalStorage_TitleUser,
        C4JStorage::TMS_FILETYPE_BINARY, C4JStorage::TMS_UGCTYPE_NONE,
        "XboxOne/SlotList", (PCHAR)pClass->m_pbSlotListFile,
        pClass->m_uiSlotListFileBytes,
    ...
    if (eStatus != C4JStorage::ETMSStatus_WriteInProgress) {
        ...
        while (pClass->m_bWaitingForWrite && (hr == S_OK)) {
            ...
            if (app.GetChangingSessionType()) {
                // 4J Stu - This causes the fullscreenprogress scene to ignore
        ...
        C4JStorage::ELoadGameStatus eLoadStatus = StorageManager.LoadSaveData(
            ...
            CScene_TransferToXboxOne::LoadSaveDataReturned, lpParameter);
        ...
        unsigned int uiPercentageChunk =
            100 / (pClass->m_uiStorageLength / uiChunkSize);
        uiComplete = 0;

        if (uiPercentageChunk == 0) uiPercentageChunk = 1;

        for (int i = 0; i < (pClass->m_uiStorageLength / uiChunkSize) + 1;
             i++) {
            ...
            if (uiBytesLeft >= uiChunkSize) {
                eStatus = StorageManager.TMSPP_WriteFile(
                    pClass->m_iPad, C4JStorage::eGlobalStorage_TitleUser,
                    C4JStorage::TMS_FILETYPE_BINARY,
                    C4JStorage::TMS_UGCTYPE_NONE, szFilename, pchData,
                    uiChunkSize, &CScene_TransferToXboxOne::TMSPPWriteReturned,
                    lpParameter, 0);
                ...
                eStatus = StorageManager.TMSPP_WriteFile(
                    ...
                    C4JStorage::TMS_FILETYPE_BINARY,
                    C4JStorage::TMS_UGCTYPE_NONE, szFilename, pchData,
                    uiBytesLeft, &CScene_TransferToXboxOne::TMSPPWriteReturned,
                    lpParameter, 0);
            }
...
int CScene_TransferToXboxOne::LoadSaveDataReturned(void* pParam,
                                                   bool bContinue) {
    CScene_TransferToXboxOne* pClass = (CScene_TransferToXboxOne*)pParam;
    ...
    if (bContinue == true) {
        ...
        StorageManager.GetSaveData(pClass->m_pvSaveMem,
                                   &pClass->m_uiStorageLength);
...
HRESULT CScene_TransferToXboxOne::OnKeyDown(XUIMessageInput* pInputData,
                                            BOOL& rfHandled) {
    if (m_bIgnoreInput) return S_OK;
            ...
            if (m_pbSlotListFile != NULL) {
                ...
                C4JStorage::ETMSStatus eStatus =
                    StorageManager.TMSPP_DeleteFile(
                        m_iPad, "XboxOne/SlotList",
                        C4JStorage::TMS_FILETYPE_BINARY,
                        &CScene_TransferToXboxOne::TMSPPDeleteReturned, this,
                        0);
            }
...
HRESULT CScene_TransferToXboxOne::OnNotifySelChanged(
...
HRESULT CScene_TransferToXboxOne::OnTransitionStart(
...
HRESULT CScene_TransferToXboxOne::OnNotifySetFocus(
...
HRESULT CScene_TransferToXboxOne::OnNotifyKillFocus(
    HXUIOBJ hObjSource, XUINotifyFocus* pNotifyFocusData, BOOL& bHandled) {
    return S_OK;
}
