...
LPCWSTR CScene_Debug::m_DebugButtonTextA[eDebugButton_Max + 1] = {
    L"Award Theme",         L"Award Avatar Item 1", L"Award Avatar Item 2",
    L"Award Avatar Item 3", L"Award Gamerpic 1",    L"Award Gamerpic 2",
    L"Check Tips",          L"Wipe Leaderboards",   L"",
};
...
HRESULT CScene_Debug::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    while ((*m_DebugCheckboxTextA[iCheckboxIndex]) != 0) {
        XuiCreateObject(XUI_CLASS_CHECKBOX,
                        &m_DebugCheckboxDataA[iCheckboxIndex].hXuiObj);
        XuiObjectFromHandle(m_DebugCheckboxDataA[iCheckboxIndex].hXuiObj,
                            &m_DebugCheckboxDataA[iCheckboxIndex].pvData);
    ...
    while ((*m_DebugButtonTextA[iButtonIndex]) != 0) {
        XuiCreateObject(XUI_CLASS_BUTTON,
                        &m_DebugButtonDataA[iButtonIndex].hXuiObj);
        XuiObjectFromHandle(m_DebugButtonDataA[iButtonIndex].hXuiObj,
                            &m_DebugButtonDataA[iButtonIndex].pvData);
        ...
        // if you're not the primary player, disable all these - you need a
        // storage device for them

        if (ProfileManager.GetPrimaryPad() != m_iPad) {
            XuiControlSetEnable(m_DebugButtonDataA[iButtonIndex].hXuiObj,
                                FALSE);
        }
        iButtonIndex++;
    }
...
HRESULT CScene_Debug::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled) {
    ...
    if (pInputData->dwKeyCode == VK_PAD_B) {
        ...
        for (int i = 0; i < m_iTotalCheckboxElements; i++) {
            CXuiCheckbox* pCheckbox =
                (CXuiCheckbox*)m_DebugCheckboxDataA[i].pvData;
            uiDebugBitmask |=
                pCheckbox->IsChecked() ? (1 << iCurrentBitmaskIndex) : 0;
            iCurrentBitmaskIndex++;
        }

        if (uiDebugBitmask !=
            app.GetGameSettingsDebugMask(pInputData->UserIndex)) {
...
HRESULT CScene_Debug::OnControlNavigate(
    ...
    if (m_bOnCheckboxes) {
            ...
            case XUI_CONTROL_NAVIGATE_UP:
                if (m_iCurrentCheckboxElement > 0) {
                    ...
                    XuiElementSetUserFocus(
                        m_DebugCheckboxDataA[m_iCurrentCheckboxElement].hXuiObj,
                        m_iPad);
                }
                break;
            case XUI_CONTROL_NAVIGATE_DOWN:
                if ((m_iCurrentCheckboxElement + 1) <
                    m_iTotalCheckboxElements) {
                    m_iCurrentCheckboxElement++;
                    XuiElementSetUserFocus(
                        m_DebugCheckboxDataA[m_iCurrentCheckboxElement].hXuiObj,
        ...
        // If it's not from the current control, ignore it
        if (m_DebugButtonDataA[m_iCurrentButtonElement].hXuiObj !=
            pControlNavigateData->hObjSource)
            return S_OK;
...
//----------------------------------------------------------------------------------
HRESULT CScene_Debug::OnNotifyValueChanged(
...
// Updates the UI when the list selection changes.
//----------------------------------------------------------------------------------
HRESULT CScene_Debug::OnNotifySelChanged(
...
//----------------------------------------------------------------------------------
HRESULT CScene_Debug::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    while ((iButton < m_iTotalButtonElements) &&
           m_DebugButtonDataA[iButton].hXuiObj != hObjPressed) {
        iButton++;
    }
    ...
    // you need to be the primary player to be able to earn these, since you
    // need a storage device
