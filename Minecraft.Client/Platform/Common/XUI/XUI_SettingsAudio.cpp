...
#include "XUI_SettingsAudio.h"

//----------------------------------------------------------------------------------
// Performs initialization tasks - retrieves controls.
//----------------------------------------------------------------------------------
HRESULT CScene_SettingsAudio::OnInit(XUIMessageInit* pInitData,
                                     BOOL& bHandled) {
    ...
    m_SliderA[SLIDER_SETTINGS_MUSIC].SetValue(
    ...
    m_SliderA[SLIDER_SETTINGS_SOUND].SetValue(
    ...
    if (ProfileManager.GetPrimaryPad() != m_iPad) {
        ...
        hr = XuiElementGetChildById(m_SliderA[SLIDER_SETTINGS_MUSIC].m_hObj,
                                    L"XuiSlider", &hSlider);
        ...
        hr = XuiElementGetChildById(m_SliderA[SLIDER_SETTINGS_SOUND].m_hObj,
                                    L"XuiSlider", &hSlider);
        XuiElementSetShow(hSlider, FALSE);
        XuiControlSetEnable(hSlider, FALSE);
    }
...
HRESULT CScene_SettingsAudio::OnKeyDown(XUIMessageInput* pInputData,
                                        BOOL& rfHandled) {
...
//----------------------------------------------------------------------------------
HRESULT CScene_SettingsAudio::OnNotifyPressEx(HXUIOBJ hObjPressed,
...
HRESULT CScene_SettingsAudio::OnControlNavigate(
...
HRESULT CScene_SettingsAudio::OnTransitionStart(
    ...
    if (pTransition->dwTransType == XUI_TRANSITION_TO ||
        ...
        hr = XuiElementGetPosition(m_SliderA[SLIDER_SETTINGS_MUSIC].m_hObj,
                                   &vSlider);
        ...
        for (int i = 0; i < SLIDER_SETTINGS_AUDIO_MAX; i++) {
            switch (i) {
                case SLIDER_SETTINGS_MUSIC:  // 3 digits
                    swprintf((WCHAR*)TempString, 256, L"%ls: %d%%",
                             app.GetString(IDS_SLIDER_MUSIC), 999);
                    break;
                case SLIDER_SETTINGS_SOUND:  // 3 digits
                    swprintf((WCHAR*)TempString, 256, L"%ls: %d%%",
                             app.GetString(IDS_SLIDER_SOUND), 999);
                    break;
            }
...
HRESULT CScene_SettingsAudio::OnCustomMessage_Splitscreenplayer(
