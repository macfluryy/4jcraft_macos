...
HRESULT CScene_LoadGameSettings::OnInit(XUIMessageInit* pInitData,
                                        BOOL& bHandled) {
    m_hXuiBrush = NULL;
    m_bSetup = false;
    ...
    // 4J-JEV: Fix for:
    // TU12: Content: Gameplay: New "Mass Effect World" remembers and uses the
    // settings of another - lately created - World.
    ...
    if (m_levelGen) {
        ...
        if (m_levelGen->requiresTexturePack()) {
            ...
            // Set this level as created in creative mode, so that people can't
            // use the themed worlds as an easy way to get achievements
        ...
        if (pbImageData == NULL) {
            ...
            app.GetImageTextData(pbImageData, dwImageBytes,
                                 (unsigned char*)&m_szSeed, uiHostOptions,
                                 bHostOptionsRead,
                                 m_MoreOptionsParams.dwTexturePack);
            XuiCreateTextureBrushFromMemory(pbImageData, dwImageBytes,
                                            &m_hXuiBrush);
            ...
            if (m_szSeed[0] != 0) {
                swprintf((WCHAR*)TempString, 256, L"%ls: %hs",
                         app.GetString(IDS_SEED), m_szSeed);
        ...
        if (bHostOptionsRead) {
            ...
            if (app.GetGameHostOption(uiHostOptions, eGameHostOption_GameType) >
                0) {
    ...
    if (app.StartInstallDLCProcess(m_iPad) == true) {
        ...
        m_currentTexturePackIndex = m_pTexturePacksList->GetIndexByUserData(
            m_MoreOptionsParams.dwTexturePack);
...
HRESULT CScene_LoadGameSettings::OnControlNavigate(
        ...
        if (m_bGameModeSurvival != true || m_bHasBeenInCreative) {
            ...
            if (m_bGameModeSurvival) {
                StorageManager.RequestMessageBox(
                    ...
                    uiIDA, 2, m_iPad,
                    &CScene_LoadGameSettings::ConfirmLoadReturned, this,
                    app.GetStringTable());
            } else  // it's creative mode
            {
                ...
                if (m_bHasBeenInCreative) {
                    ...
                    // StorageManager.RequestMessageBox(IDS_TITLE_START_GAME,
                    ...
                    if (m_levelGen != NULL) {
                        ...
                        C4JStorage::ELoadGameStatus eLoadStatus =
                            StorageManager.LoadSaveData(
                                &m_XContentData,
                                CScene_LoadGameSettings::LoadSaveDataReturned,
                                this);
            ...
            StorageManager.RequestMessageBox(
                ...
                2, m_iPad, &CScene_LoadGameSettings::ConfirmLoadReturned, this,
                app.GetStringTable());
        }
    } else {
        if (m_levelGen != NULL) {
            ...
            C4JStorage::ELoadGameStatus eLoadStatus =
                StorageManager.LoadSaveData(
                    &m_XContentData,
                    CScene_LoadGameSettings::LoadSaveDataReturned, this);
...
HRESULT CScene_LoadGameSettings::OnNotifyPressEx(
    ...
    if (hObjPressed == m_ButtonLoad) {
        ...
        if (m_MoreOptionsParams.dwTexturePack != 0) {
            ...
            if (pTexturePack == NULL) {
                ...
                // 4J-PB - if the full offer id is 0, then the texture pack dlc
                // load failed
                if (ullOfferID_Full != 0LL) {
                    ...
                    if (pDLCInfo->ullOfferID_Trial != 0LL) {
                        ...
                        StorageManager.RequestMessageBox(
                            ...
                            ProfileManager.GetPrimaryPad(),
                            &CScene_LoadGameSettings::TexturePackDialogReturned,
            ...
            if (pTexturePack == NULL) {
                ...
                if (m_pDLCPack && !m_pDLCPack->hasPurchasedFile(
                    ...
                    if (!ProfileManager.IsSignedInLive(
                        ...
                        // StorageManager.RequestMessageBox(IDS_UNLOCK_DLC_TITLE,
                        // IDS_UNLOCK_DLC_SKIN, uiIDA, 2,
                        // pInputData->UserIndex,&CScene_SkinSelect::UnlockSkinReturned,this,app.GetStringTable());
                        StorageManager.RequestMessageBox(
                            IDS_UNLOCK_DLC_TEXTUREPACK_TITLE,
                            IDS_UNLOCK_DLC_TEXTUREPACK_TEXT, uiIDA, 2,
                            pNotifyPressData->UserIndex,
                            &CScene_LoadGameSettings::UnlockTexturePackReturned,
                            this, app.GetStringTable());
                        return S_OK;
                    }
                }
        ...
        // Check if they have the Reset Nether flag set, and confirm they want
        // to do this
        if (m_MoreOptionsParams.bResetNether) {
            ...
            StorageManager.RequestMessageBox(
                IDS_RESETNETHER_TITLE, IDS_RESETNETHER_TEXT, uiIDA, 2,
                pNotifyPressData->UserIndex,
                &CScene_LoadGameSettings::CheckResetNetherReturned, this,
                app.GetStringTable());
        } else {
            LaunchGame();
        }
...
HRESULT CScene_LoadGameSettings::OnKeyDown(XUIMessageInput* pInputData,
                                           BOOL& rfHandled) {
    if (m_bIgnoreInput) return S_OK;
    ...
    switch (pInputData->dwKeyCode) {
        ...
        case VK_PAD_RSHOULDER:
            /*		if(app.m_bTransferSavesToXboxOne &&
...
HRESULT CScene_LoadGameSettings::OnFontRendererChange() {
    ...
    ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                   IDS_TOOLTIPS_BACK, -1, -1, -1, -1, -1, -1, -1, true);

    return S_OK;
}

int CScene_LoadGameSettings::ConfirmLoadReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        if (pClass->m_levelGen != NULL) {
            ...
            C4JStorage::ELoadGameStatus eLoadStatus =
                StorageManager.LoadSaveData(
                    &pClass->m_XContentData,
                    CScene_LoadGameSettings::LoadSaveDataReturned, pClass);
...
HRESULT CScene_LoadGameSettings::OnTimer(XUIMessageTimer* pTimer,
                                         BOOL& bHandled) {
                ...
                if (m_iConfigA[i] != -1) {
                    ...
                    if (dwBytes > 0 && pbData) {
                        ...
                        m_currentTexturePackIndex =
                            m_pTexturePacksList->GetIndexByUserData(
                                m_MoreOptionsParams.dwTexturePack);
                        m_pTexturePacksList->SetCurSel(
                            m_currentTexturePackIndex);
                        m_pTexturePacksList->SetTopItem(
                            m_currentTexturePackIndex);  // scroll the item into
                                                         // view if it's not
                                                         // visible
                    }
                }
...
int CScene_LoadGameSettings::LoadSaveDataReturned(void* pParam,
                                                  bool bContinue) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
    ...
    if (bContinue == true) {
        ...
        if (!isClientSide || connectedControllers == 1 ||
            ...
            ProfileManager.RequestSignInUI(
                ...
                &CScene_LoadGameSettings::StartGame_SignInReturned, pParam,
                ProfileManager.GetPrimaryPad());
        }
    } else {
        // the save is corrupt!
        ...
        StorageManager.RequestMessageBox(
            ...
            &CScene_LoadGameSettings::DeleteSaveDialogReturned, pClass,
            app.GetStringTable());
    }
    return 0;
}

int CScene_LoadGameSettings::DeleteSaveDialogReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
    ...
    if (result == C4JStorage::EMessage_ResultDecline) {
        ...
        StorageManager.DeleteSaveData(
            &pClass->m_XContentData,
            CScene_LoadGameSettings::DeleteSaveDataReturned, pClass);
#endif
    } else {
        pClass->m_bIgnoreInput = false;
    }
...
int CScene_LoadGameSettings::DeleteSaveDataReturned(void* pParam,
                                                    bool bSuccess) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
...
int CScene_LoadGameSettings::DeviceRemovedDialogReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
...
// sign-in or not
void CScene_LoadGameSettings::StartGameFromSave(CScene_LoadGameSettings* pClass,
...
int CScene_LoadGameSettings::StartGame_SignInReturned(void* pParam,
                                                      bool bContinue,
                                                      int iPad) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
...
HRESULT CScene_LoadGameSettings::OnGetSourceDataImage(
...
HRESULT CScene_LoadGameSettings::OnNotifyValueChanged(
...
HRESULT CScene_LoadGameSettings::OnTransitionStart(
    ...
    if (pTransition->dwTransAction == XUI_TRANSITION_ACTION_DESTROY ||
        ...
        // while you were on the More Options scene
...
HRESULT CScene_LoadGameSettings::OnTransitionEnd(
    XUIMessageTransition* pTransition, BOOL& bHandled) {
    // if(pTransition->dwTransAction==XUI_TRANSITION_ACTION_DESTROY ) return
    // S_OK;
...
int CScene_LoadGameSettings::UnlockTexturePackReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
...
HRESULT CScene_LoadGameSettings::OnNotifySelChanged(
...
HRESULT CScene_LoadGameSettings::OnNotifyKillFocus(
    ...
    if (tp == NULL) {
        ...
        if (pDLCInfo->ullOfferID_Trial != 0LL) {
            ...
            StorageManager.RequestMessageBox(
                ...
                IDS_DLC_TEXTUREPACK_NOT_PRESENT, uiIDA, 3,
                ProfileManager.GetPrimaryPad(),
                &CScene_LoadGameSettings::TexturePackDialogReturned, this,
                app.GetStringTable());
        } else {
            uiIDA[0] = IDS_TEXTUREPACK_FULLVERSION;
            uiIDA[1] = IDS_CONFIRM_CANCEL;
...
void CScene_LoadGameSettings::LoadLevelGen(LevelGenerationOptions* levelGen) {
    ...
    bool isPrivate = (app.GetGameSettings(m_iPad, eGameSetting_InviteOnly) > 0)
                         ? true
                         : false;
    ...
    app.SetGameHostOption(
        eGameHostOption_Gamertags,
        app.GetGameSettings(m_iPad, eGameSetting_GamertagsVisible));

    app.SetGameHostOption(
        eGameHostOption_BedrockFog,
        app.GetGameSettings(m_iPad, eGameSetting_BedrockFog) ? 1 : 0);

    // 4J-JEV: Fix for:
    // TU12: Content: Gameplay: New "Mass Effect World" remembers and uses the
    // settings of another - lately created - World.
    app.SetGameHostOption(eGameHostOption_LevelType,
                          m_MoreOptionsParams.bFlatWorld);
    app.SetGameHostOption(eGameHostOption_Structures,
                          m_MoreOptionsParams.bStructures);
    app.SetGameHostOption(eGameHostOption_BonusChest,
                          m_MoreOptionsParams.bBonusChest);
    ...
    app.SetGameHostOption(eGameHostOption_GameType,
                          m_bGameModeSurvival ? GameType::SURVIVAL->getId()
                                              : GameType::CREATIVE->getId());
...
HRESULT CScene_LoadGameSettings::OnCustomMessage_DLCMountingComplete() {
    ...
    m_pTexturePacksList->SetCurSel(m_currentTexturePackIndex);
    m_pTexturePacksList->SetTopItem(
        m_currentTexturePackIndex);  // scroll the item into view if it's not
                                     // visible
...
int CScene_LoadGameSettings::TexturePackDialogReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    CScene_LoadGameSettings* pClass = (CScene_LoadGameSettings*)pParam;
#ifdef _XBOX
...
#endif
    CXuiSceneBase::SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                               IDS_TOOLTIPS_BACK, -1, -1, -1, -1, -1, iRB);
    return S_OK;
}
