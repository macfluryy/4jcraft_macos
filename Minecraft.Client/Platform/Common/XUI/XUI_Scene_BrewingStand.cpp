...
//--------------------------------------------------------------------------------------
HRESULT CXuiSceneBrewingStand::OnInit(XUIMessageInit* pInitData,
                                      BOOL& bHandled) {
    MapChildControls();
    ...
    CXuiSceneAbstractContainer::Initialize(
        m_iPad, menu, true, BrewingStandMenu::INV_SLOT_START,
...
void CXuiSceneBrewingStand::InitDataAssociations(int iPad,
                                                 AbstractContainerMenu* menu,
                                                 int startIndex /*= 0*/) {
    // TODO Inventory dimensions need defined as constants
    m_ingredientControl->SetData(iPad, menu, 1, 1,
                                 BrewingStandMenu::INGREDIENT_SLOT);

    m_bottle1Control->SetData(iPad, menu, 1, 1,
                              BrewingStandMenu::BOTTLE_SLOT_START);
    m_bottle2Control->SetData(iPad, menu, 1, 1,
                              BrewingStandMenu::BOTTLE_SLOT_START + 1);
    m_bottle3Control->SetData(iPad, menu, 1, 1,
                              BrewingStandMenu::BOTTLE_SLOT_START + 2);
