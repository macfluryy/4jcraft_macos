...
HRESULT CScene_Reinstall::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_THEME],
                      app.GetString(IDS_REINSTALL_THEME));
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_GAMERPIC1],
                      app.GetString(IDS_REINSTALL_GAMERPIC_1));
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_GAMERPIC2],
                      app.GetString(IDS_REINSTALL_GAMERPIC_2));
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_AVATAR1],
                      app.GetString(IDS_REINSTALL_AVATAR_ITEM_1));
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_AVATAR2],
                      app.GetString(IDS_REINSTALL_AVATAR_ITEM_2));
    XuiControlSetText(m_Buttons[BUTTON_REINSTALL_AVATAR3],
                      app.GetString(IDS_REINSTALL_AVATAR_ITEM_3));

    int iFirstEnabled = -1;

    // we can only come in here if we are the primary player, it's the full
    // version, and we have some content to re-install
...
//----------------------------------------------------------------------------------
HRESULT CScene_Reinstall::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    while ((uiButtonCounter < BUTTONS_REINSTALL_MAX) &&
    ...
    if (!StorageManager.GetSaveDeviceSelected(m_iPad) ||
        ...
        // Fix for #12531 - TCR 001: BAS Game Stability: When a player selects
        // to change a storage device, and repeatedly backs out of the SD
        // screen, disconnects from LIVE, and then selects a SD, the title
        // crashes.
        m_bIgnoreInput = true;

        StorageManager.SetSaveDevice(
            &CScene_Reinstall::DeviceSelectReturned_AndReinstall, this, true);
        return S_OK;
    }
    ...
    switch (uiButtonCounter) {
        ...
        case BUTTON_REINSTALL_GAMERPIC1:
            ProfileManager.Award(pNotifyPressData->UserIndex,
                                 eAward_mine100Blocks, true);
            break;
        case BUTTON_REINSTALL_GAMERPIC2:
            ProfileManager.Award(pNotifyPressData->UserIndex,
                                 eAward_kill10Creepers, true);
            break;
        case BUTTON_REINSTALL_AVATAR1:
            ProfileManager.Award(pNotifyPressData->UserIndex,
                                 eAward_eatPorkChop, true);
            break;
        case BUTTON_REINSTALL_AVATAR2:
            ProfileManager.Award(pNotifyPressData->UserIndex,
                                 eAward_play100Days, true);
            break;
        case BUTTON_REINSTALL_AVATAR3:
            ProfileManager.Award(pNotifyPressData->UserIndex,
                                 eAward_arrowKillCreeper, true);
            break;
    }
...
HRESULT CScene_Reinstall::OnKeyDown(XUIMessageInput* pInputData,
                                    BOOL& rfHandled) {
    if (m_bIgnoreInput) return S_OK;
    ...
    switch (pInputData->dwKeyCode) {
        ...
        case VK_PAD_X:
            ...
            // changes, we need to update the tooltips Fix for #12531 - TCR 001:
            // BAS Game Stability: When a player selects to change a storage
            // device, and repeatedly backs out of the SD screen, disconnects
            // from LIVE, and then selects a SD, the title crashes.
            m_bIgnoreInput = true;
            StorageManager.SetSaveDevice(
                &CScene_Reinstall::DeviceSelectReturned, this, true);
...
HRESULT CScene_Reinstall::OnControlNavigate(
...
HRESULT CScene_Reinstall::OnTransitionStart(XUIMessageTransition* pTransition,
                                            BOOL& bHandled) {
...
int CScene_Reinstall::DeviceSelectReturned_AndReinstall(void* pParam,
                                                        bool bContinue) {
    CScene_Reinstall* pClass = (CScene_Reinstall*)pParam;
    ...
    if (!StorageManager.GetSaveDeviceSelected(pClass->m_iPad)) {
        ...
        switch (pClass->m_iLastButtonPressed) {
            case BUTTON_REINSTALL_THEME:
                ...
                break;
            case BUTTON_REINSTALL_GAMERPIC1:
                ProfileManager.Award(pClass->m_iPad, eAward_mine100Blocks,
                                     true);
                break;
            case BUTTON_REINSTALL_GAMERPIC2:
                ProfileManager.Award(pClass->m_iPad, eAward_kill10Creepers,
                                     true);
                break;
            case BUTTON_REINSTALL_AVATAR1:
            ...
            case BUTTON_REINSTALL_AVATAR2:
                ...
                break;
            case BUTTON_REINSTALL_AVATAR3:
                ProfileManager.Award(pClass->m_iPad, eAward_arrowKillCreeper,
                                     true);
                break;
        }
    }
