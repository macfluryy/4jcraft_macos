...
HRESULT CScene_DLCMain::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    // if(app.GetTMSDLCInfoRead())
    {
        ...
        CXuiCtrl4JList::LIST_ITEM_INFO* pListInfo =
            new CXuiCtrl4JList::LIST_ITEM_INFO[e_DLC_MAX_MinecraftStore];
        ZeroMemory(pListInfo, sizeof(CXuiCtrl4JList::LIST_ITEM_INFO) *
                                  e_DLC_MAX_MinecraftStore);

        m_bAllDLCContentRetrieved = false;
        pListInfo[e_DLC_SkinPack].pwszText =
            app.GetString(IDS_DLC_MENU_SKINPACKS);
        pListInfo[e_DLC_SkinPack].fEnabled = TRUE;
        list->AddData(pListInfo[e_DLC_SkinPack]);

        pListInfo[e_DLC_TexturePacks].pwszText =
        ...
        pListInfo[e_DLC_MashupPacks].pwszText =
            app.GetString(IDS_DLC_MENU_MASHUPPACKS);
        ...
        pListInfo[e_DLC_AvatarItems].pwszText =
            app.GetString(IDS_DLC_MENU_AVATARITEMS);
        pListInfo[e_DLC_AvatarItems].fEnabled = TRUE;
        list->AddData(pListInfo[e_DLC_AvatarItems]);

        pListInfo[e_DLC_Gamerpics].pwszText =
            app.GetString(IDS_DLC_MENU_GAMERPICS);
        pListInfo[e_DLC_Gamerpics].fEnabled = TRUE;
        list->AddData(pListInfo[e_DLC_Gamerpics]);

        app.AddDLCRequest(
            e_Marketplace_Content);  // content is skin packs, texture packs and
                                     // mash-up packs
    ...
    TelemetryManager->RecordMenuShown(iPad, eUIScene_DLCMainMenu,
                                      0);  // 4J JEV ?

    return S_OK;
}

HRESULT CScene_DLCMain::OnDestroy() { return S_OK; }
...
HRESULT CScene_DLCMain::OnKeyDown(XUIMessageInput* pInputData,
                                  BOOL& rfHandled) {
    ...
    switch (pInputData->dwKeyCode) {
        ...
        case VK_ESCAPE:
            ...
            // set the timer running to navigate back when any tms retrieval has
            // come in
...
HRESULT CScene_DLCMain::OnNotifyPressEx(HXUIOBJ hObjPressed,
...
HRESULT CScene_DLCOffers::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    // If this is the avatar items, we need to init the avatar system to allow
    // us to find out what the player avatar gender is (since we should only
    // display tshirts etc with the right gender) The init reserves 3MB of
    // memory, so we shut down on leaving this.
...
HRESULT CScene_DLCOffers::GetDLCInfo(int iOfferC, bool bUpdateOnly) {
    ...
    if (bUpdateOnly)  // Just update the info on the current list
    {
        for (int i = 0; i < iOfferC; i++) {
            ...
            // If the item has a gender, then skip ones that are the other
            // gender
            ...
            // can't trust the offer type - partnernet is giving avatar items
            // the CONTENT type
                ...
                if (xOffer.fUserHasPurchased) {
                    ...
                    if (RenderManager.IsHiDef() ||
                        ...
                        swprintf(szResourceLocator, LOCATOR_SIZE,
                                 L"section://%X,%ls#%ls", c_ModuleHandle,
                                 L"media", L"media/Graphics/DLC_Tick.png");
                        XuiCreateTextureBrush(szResourceLocator, &hBrush);
                    } else {
                        swprintf(szResourceLocator, LOCATOR_SIZE,
                                 L"section://%X,%ls#%ls", c_ModuleHandle,
                                 L"media", L"media/Graphics/DLC_TickSmall.png");
                        XuiCreateTextureBrush(szResourceLocator, &hBrush);
                    }
        ...
        if (iOfferC != 0) {
            ...
            ZeroMemory(pListInfo,
                       sizeof(CXuiCtrl4JList::LIST_ITEM_INFO) * iOfferC);
        }
        ...
        for (int i = 0; i < iOfferC; i++) {
            ...
            if (pDLC->eDLCType == (eDLCContentType)m_iType) {
                ...
                // Bug 49249 - JPN: Code Defect: Missing Text: String
                // 'Minecraft' is missing in contents download screen. Looks
                // like we shouldn't be removing this text for Japanese, and
                // probably Chinese & Korean
                ...
                switch (dwLanguage) {
                    ...
                    default:
                        if (wstrTemp.compare(0, 10, L"Minecraft ") == 0) {
                            pListInfo[iCount].pwszText =
                                &xOffer.wszOfferName[10];
                        } else {
                            pListInfo[iCount].pwszText = xOffer.wszOfferName;
                        }
                        break;
                ...
                m_pOffersList->AddData(pListInfo[iCount], 0,
    ...
    m_Timer.SetShow(FALSE);
    if (iCount != 0) {
        // get the right index for the first list item - it will have been
        // re-sorted internally in the list
        int iIndex = 0;
        ...
        if (dlc != NULL) {
            ...
            if (iIndex != -1) {
                ...
                XuiCreateTextureBrushFromMemory(app.TMSFileA[iIndex].pbData,
                                                app.TMSFileA[iIndex].uiSize,
                                                &m_hXuiBrush);
            } else {
                bool bPresent = app.IsFileInMemoryTextures(cString);
                if (!bPresent) {
                    // Image has not come in yet
                    // Set the item monitored in the timer, so we can set the
                    ...
                    XuiCreateTextureBrushFromMemory(pData, dwSize,
                                                    &m_hXuiBrush);
                }
            }
        }
...
//----------------------------------------------------------------------------------
HRESULT CScene_DLCOffers::OnNotifyPressEx(HXUIOBJ hObjPressed,
                                          XUINotifyPress* pNotifyPressData,
                                          BOOL& rfHandled) {
    ...
    if (hObjPressed == m_List) {
        ...
        // 		}
        // 		else
        // if it's already been purchased, we need to let the user download it
        // anyway
        {
...
HRESULT CScene_DLCOffers::OnKeyDown(XUIMessageInput* pInputData,
                                    BOOL& rfHandled) {
    // Fix for Compliance fail -
    // On a functional console, the game must not enter an extended unresponsive
    // state, cause unintentional loss of player data, crash, or cause an
    // unintended reboot of the machine.
...
void CScene_DLCOffers::UpdateTooltips(
    XMARKETPLACE_CURRENCY_CONTENTOFFER_INFO& xOffer) {
    // if the current offer hasn't been purchased already, check if there's a
    // trial version available
    if (xOffer.fUserHasPurchased == false) {
        ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_INSTALL,
                       IDS_TOOLTIPS_BACK);
    } else {
        ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_REINSTALL,
                       IDS_TOOLTIPS_BACK);
    }
}

HRESULT CScene_DLCOffers::OnGetSourceDataImage(
...
HRESULT CScene_DLCOffers::OnNotifySelChanged(
    ...
    if (m_List.TreeHasFocus())  // && offerIndexes.size() > index)
    {
        ...
        if (dlc != NULL) {
            ...
            if (iIndex != -1) {
                ...
                if (!bPresent) {
                    ...
                    // promote it to the top of the queue of images to be
                    // retrieved We can't trust the dwContentCategory from
                    // partnernet - it has avatar items as content instead of
                    // avatars
                    ...
                    XuiCreateTextureBrushFromMemory(pImage, dwSize,
                                                    &m_hXuiBrush);
                }
            }
        } else {
        ...
        ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_ACCEPT,
                       IDS_TOOLTIPS_BACK);
    }
    return S_OK;
}
...
HRESULT CScene_DLCOffers::OnTimer(XUIMessageTimer* pData, BOOL& rfHandled) {
    ...
    if (ProfileManager.GetLiveConnectionStatus() !=
        ...
        StorageManager.RequestMessageBox(
            IDS_CONNECTION_LOST, IDS_CONNECTION_LOST_LIVE, uiIDA, 1,
            ProfileManager.GetPrimaryPad(),
            &CConsoleMinecraftApp::EthernetDisconnectReturned, this,
            app.GetStringTable());
    }
...
// int CScene_DLCOffers::EthernetDisconnectReturned(void *pParam,int iPad,const
