...
HRESULT CScene_Main::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    XuiControlSetText(m_Buttons[BUTTON_LEADERBOARDS],
                      app.GetString(IDS_LEADERBOARDS));
    XuiControlSetText(m_Buttons[BUTTON_ACHIEVEMENTS],
                      app.GetString(IDS_ACHIEVEMENTS));
    XuiControlSetText(m_Buttons[BUTTON_HELPANDOPTIONS],
                      app.GetString(IDS_HELP_AND_OPTIONS));
    XuiControlSetText(m_Buttons[BUTTON_UNLOCKFULLGAME],
                      app.GetString(IDS_UNLOCK_FULL_GAME));
    ...
    if (ProfileManager.IsFullVersion()) {
        ...
        m_Buttons[BUTTON_UNLOCKFULLGAME].SetText(
            app.GetString(IDS_DOWNLOADABLECONTENT));
        XuiElementSetShow(m_Buttons[BUTTON_UNLOCKFULLGAME], TRUE);
    }

    // Do we have downloadable content? - We need to have this in for a Pre-Cert
    // test, whether or not we have DLC at the time.
    ...
    swprintf(szResourceLocator, LOCATOR_SIZE, L"section://%X,%ls#%ls",
             c_ModuleHandle, L"media", L"media/splashes.txt");

    BYTE* splashesData;
    UINT splashesSize;
    hr = XuiResourceLoadAllNoLoc(szResourceLocator, &splashesData,
                                 &splashesSize);
    ...
    if (HRESULT_SUCCEEDED(hr)) {
        ...
        XuiFree(
            splashesData);  // Frees copy returned from XuiResourceLoadAllNoLoc
...
HRESULT CScene_Main::OnNotifySetFocus(HXUIOBJ hObjSource,
                                      XUINotifyFocus* pNotifyFocusData,
                                      BOOL& bHandled) {
    // If we sign out when in the saves list, we get a notifysetfocus in the
    // saves list after the init of the main menu
    ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT);
...
//----------------------------------------------------------------------------------
HRESULT CScene_Main::OnNotifyPressEx(HXUIOBJ hObjPressed,
                                     XUINotifyPress* pNotifyPressData,
                                     BOOL& rfHandled) {
    // should we ignore the button press? This will be set if we're waiting for
    // a callback from a function launched from a button press
    ...
    while ((uiButtonCounter < BUTTONS_MAX) &&
    ...
    switch (uiButtonCounter) {
        case BUTTON_PLAYGAME:
            ...
            ProfileManager.SetCurrentGameActivity(pNotifyPressData->UserIndex,
                                                  CONTEXT_PRESENCE_MENUS, true);
            ...
            if (ProfileManager.IsSignedIn(pNotifyPressData->UserIndex)) {
                ...
                StorageManager.RequestMessageBox(
                    ...
                    &CScene_Main::MustSignInReturned, this,
                    app.GetStringTable());
            }
        ...
        case BUTTON_LEADERBOARDS:
            ...
            if (ProfileManager.IsSignedIn(pNotifyPressData->UserIndex)) {
                ...
                // &CScene_Main::Leaderboards_SignInReturned, this);
                // get them to sign in
                unsigned int uiIDA[2];
                uiIDA[0] = IDS_CONFIRM_OK;
                uiIDA[1] = IDS_CONFIRM_CANCEL;
        ...
        case BUTTON_ACHIEVEMENTS:
            ...
            if (ProfileManager.IsSignedIn(pNotifyPressData->UserIndex)) {
                ...
                // get them to sign in
                // ProfileManager.RequestSignInUI(false, false,
                // true,false,true,&CScene_Main::Achievements_SignInReturned,this
                // );
                // get them to sign in
                unsigned int uiIDA[2];
                uiIDA[0] = IDS_CONFIRM_OK;
        ...
        case BUTTON_HELPANDOPTIONS:
            ...
            if (ProfileManager.IsSignedIn(pNotifyPressData->UserIndex)) {
                ...
                // get them to sign in
                // ProfileManager.RequestSignInUI(false, false,
                // true,false,true,&CScene_Main::HelpAndOptions_SignInReturned,this
                // );
                // get them to sign in
                unsigned int uiIDA[2];
                uiIDA[0] = IDS_CONFIRM_OK;
        ...
        case BUTTON_EXITGAME:
            if (ProfileManager.IsFullVersion()) {
                ...
                StorageManager.RequestMessageBox(
                    ...
                    XUSER_INDEX_ANY, &CScene_Main::ExitGameReturned, this);
            } else {
...
HRESULT CScene_Main::OnTransitionStart(XUIMessageTransition* pTransition,
                                       BOOL& bHandled) {
    ...
    if (pTransition->dwTransType == XUI_TRANSITION_TO ||
        ...
        // 4J-PB - remove the "hobo humping" message legal (Sony) say we can't
        // have - pretty sure Microsoft would say the same if they noticed it.
        int splashIndex =
...
HRESULT CScene_Main::OnControlNavigate(
...
int CScene_Main::SignInReturned(void* pParam, bool bContinue) {
    ...
    if (bContinue == true) {
        StorageManager.SetSaveDevice(&CScene_Main::DeviceSelectReturned,
                                     pClass);
    }
...
int CScene_Main::DeviceSelectReturned(void* pParam, bool bContinue) {
    ...
    if (bContinue == true) {
        ...
        XuiSetTimer(pClass->m_hObj, DLC_INSTALLED_TIMER_ID,
...
int CScene_Main::CreateLoad_OfflineProfileReturned(void* pParam, bool bContinue,
                                                   int iPad) {
    CScene_Main* pClass = (CScene_Main*)pParam;
    ...
    if (bContinue == true) {
        ...
        if (ProfileManager.IsFullVersion()) {
            if (StorageManager.SetSaveDevice(&CScene_Main::DeviceSelectReturned,
                                             pClass)) {
        ...
        // force a sign-in - they were offline, and they want to be online, so
        // don't let it display offline players set the bAddUser to false to
        // allow offline to go online by selecting the already signed in player
        // again
        ProfileManager.RequestSignInUI(false, false, true, false, false,
                                       &CScene_Main::CreateLoad_SignInReturned,
...
int CScene_Main::CreateLoad_SignInReturned(void* pParam, bool bContinue,
                                           int iPad) {
    CScene_Main* pClass = (CScene_Main*)pParam;
    ...
    if (bContinue == true) {
        ...
        if (ProfileManager.IsGuest(ProfileManager.GetPrimaryPad())) {
            StorageManager.RequestMessageBox(IDS_PRO_GUESTPROFILE_TITLE,
...
int CScene_Main::MustSignInReturned(void* pParam, int iPad,
        ...
        switch (pClass->m_eAction) {
            case eAction_RunGame:
                ProfileManager.RequestSignInUI(
                    ...
                    &CScene_Main::CreateLoad_SignInReturned, pClass, iPad);
                break;
            case eAction_RunLeaderboards:
                ProfileManager.RequestSignInUI(
                    false, false, true, false, true,
                    &CScene_Main::Leaderboards_SignInReturned, pClass, iPad);
                break;
            case eAction_RunAchievements:
                ProfileManager.RequestSignInUI(
                    false, false, true, false, true,
                    &CScene_Main::Achievements_SignInReturned, pClass, iPad);
                break;
            case eAction_RunHelpAndOptions:
                ProfileManager.RequestSignInUI(
                    false, false, true, false, true,
                    &CScene_Main::HelpAndOptions_SignInReturned, pClass, iPad);
                break;
            case eAction_RunUnlockOrDLC:
                ProfileManager.RequestSignInUI(
                    false, false, true, false, true,
                    &CScene_Main::UnlockFullGame_SignInReturned, pClass, iPad);
                break;
        }
    } else {
        // unlock the profile
...
int CScene_Main::AchievementsDeviceSelectReturned(void* pParam,
                                                  bool bContinue) {
    // CScene_Main* pClass = (CScene_Main*)pParam;
    // HRESULT hr;
...
int CScene_Main::Leaderboards_SignInReturned(void* pParam, bool bContinue,
                                             int iPad) {
    // CScene_Main* pClass = (CScene_Main*)pParam;
    ...
    if (bContinue == true) {
        ...
        if (ProfileManager.IsGuest(ProfileManager.GetPrimaryPad())) {
            StorageManager.RequestMessageBox(IDS_PRO_GUESTPROFILE_TITLE,
                       ...
                       ProfileManager.GetPrimaryPad())) {
            StorageManager.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE,
                                             IDS_PRO_XBOXLIVE_NOTIFICATION,
                                             uiIDA, 1);
        } else {
            ProfileManager.SetLockedProfile(ProfileManager.GetPrimaryPad());
...
int CScene_Main::Achievements_SignInReturned(void* pParam, bool bContinue,
                                             int iPad) {
    // CScene_Main* pClass = (CScene_Main*)pParam;
...
int CScene_Main::HelpAndOptions_SignInReturned(void* pParam, bool bContinue,
                                               int iPad) {
    CScene_Main* pClass = (CScene_Main*)pParam;
...
int CScene_Main::UnlockFullGame_SignInReturned(void* pParam, bool bContinue,
                                               int iPad) {
    CScene_Main* pClass = (CScene_Main*)pParam;
...
int CScene_Main::ExitGameReturned(void* pParam, int iPad,
                                  C4JStorage::EMessageResult result) {
    // CScene_Main* pClass = (CScene_Main*)pParam;
...
void CScene_Main::LoadTrial(void) {
    ...
    if (generators->empty()) {
        app.DebugPrintf(
            "XUI LoadTrial: no level generators available, cannot start "
            "tutorial\n");
        delete param;
        return;
    }
...
void CScene_Main::RunPlayGame(int iPad) {
    ...
    if (ProfileManager.IsGuest(iPad)) {
        ...
        if (ProfileManager.IsFullVersion()) {
            ...
            if (!ProfileManager.IsSignedInLive(iPad)) {
                ProfileManager.DisplayOfflineProfile(
                    &CScene_Main::CreateLoad_OfflineProfileReturned, this,
                    iPad);
            } else {
                ...
                if (app.GetTMSDLCInfoRead() && app.GetTMSXUIDsFileRead() &&
                    ...
                    // flag the timer to start the TMS calls when there is
                    // nothing happening with TMS. fix for X360 - 162325 - TCR
                    // 001: BAS Game Stability: TU17: The game goes into an
                    // infinite loading upon entering the Play Game menu shortly
                    // after visiting the Minecraft Store
...
void CScene_Main::RunLeaderboards(int iPad) {
    ...
    if (ProfileManager.IsGuest(iPad)) {
        ...
        StorageManager.RequestMessageBox(
            IDS_PRO_NOTONLINE_TITLE, IDS_PRO_XBOXLIVE_NOTIFICATION, uiIDA, 1);
    } else {
        ProfileManager.SetLockedProfile(iPad);
        // If the player was signed in before selecting play, we'll not have
        // read the profile yet, so query the sign-in status to get this to
...
void CScene_Main::RunUnlockOrDLC(int iPad) {
    ...
    if (ProfileManager.IsFullVersion()) {
        ...
        if (ProfileManager.IsGuest(iPad)) {
            StorageManager.RequestMessageBox(
                IDS_UNLOCK_TITLE, IDS_UNLOCK_GUEST_TEXT, uiIDA, 1, iPad);
        } else if (!ProfileManager.IsSignedInLive(iPad)) {
            StorageManager.RequestMessageBox(IDS_PRO_NOTONLINE_TITLE,
                                             IDS_PRO_XBOXLIVE_NOTIFICATION,
                                             uiIDA, 1);
...
int CScene_Main::TMSReadFileListReturned(
    void* pParam, int iPad, C4JStorage::PTMSPP_FILE_LIST pTmsFileList) {
    CScene_Main* pClass = (CScene_Main*)pParam;

    // push the file details in to a unordered map if they are not already in
    // there
...
int CScene_Main::TMSFileWriteReturned(void* pParam, int iPad, int iResult) {
    ...
    // push the file details in to a unordered map if they are not already in
    // there 	for(int i=0;i<pTmsFileList->iCount;i++)
    // 	{
    // 		app.PutTMSPP_FileSize(filenametostd::wstring(pTmsFileList->FileDetailsA[i].szFilename),pTmsFileList->FileDetailsA[i].iFileSize);
    // 	}
    return 0;
...
int CScene_Main::TMSFileReadReturned(void* pParam, int iPad,
                                     C4JStorage::PTMSPP_FILEDATA pData) {
    CScene_Main* pClass = (CScene_Main*)pParam;
...
HRESULT CScene_Main::OnTimer(XUIMessageTimer* pTimer, BOOL& bHandled) {
    ...
    if (pTimer->nId == DLC_INSTALLED_TIMER_ID) {
        ...
        if (app.GetTMSAction(ProfileManager.GetPrimaryPad()) ==
            eTMSAction_Idle) {
