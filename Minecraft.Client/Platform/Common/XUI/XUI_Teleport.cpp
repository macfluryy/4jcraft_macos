...
#include "../../Minecraft.Client/Commands/TeleportCommand.h"

//----------------------------------------------------------------------------------
// Performs initialization tasks - retrieves controls.
//----------------------------------------------------------------------------------
HRESULT CScene_Teleport::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    TeleportMenuInitData* initParam =
        (TeleportMenuInitData*)pInitData->pvInitData;
    ...
    g_NetworkManager.RegisterPlayerChangedCallback(
        m_iPad, &CScene_Teleport::OnPlayerChanged, this);
...
HRESULT CScene_Teleport::OnKeyDown(XUIMessageInput* pInputData,
                                   BOOL& rfHandled) {
...
//----------------------------------------------------------------------------------
HRESULT CScene_Teleport::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    if (hObjPressed == playersList) {
        ...
        if (m_teleportToPlayer) {
            packet = TeleportCommand::preparePacket(thisPlayer->GetUID(),
                                                    selectedPlayer->GetUID());
        } else {
            packet = TeleportCommand::preparePacket(selectedPlayer->GetUID(),
                                                    thisPlayer->GetUID());
        }
...
void CScene_Teleport::OnPlayerChanged(void* callbackParam,
...
HRESULT CScene_Teleport::OnGetSourceDataText(
...
HRESULT CScene_Teleport::OnGetSourceDataImage(
...
HRESULT CScene_Teleport::OnGetItemCountAll(
...
HRESULT CScene_Teleport::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                           BOOL& bHandled) {
    bHandled = true;
