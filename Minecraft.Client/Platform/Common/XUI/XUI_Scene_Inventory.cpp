...
HRESULT CXuiSceneInventory::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    InventoryMenu* menu =
        dynamic_cast<InventoryMenu*>(initData->player->inventoryMenu);
    ...
    CXuiSceneAbstractContainer::Initialize(
        initData->iPad, menu, false, InventoryMenu::INV_SLOT_START,
        eSectionInventoryUsing, eSectionInventoryMax, initData->bNavigateBack);

    delete initData;

    float fWidth;
    m_effectsGroup.GetBounds(
        &fWidth,
        &m_effectAreaHeight);  // Get total height available for effects display
    m_hEffectDisplayA[0]->GetBounds(
        &fWidth, &m_effectDisplayHeight);  // Get height of one effect
    ...
    m_effectDisplaySpacing =
        firstEffectPos.y - secondEffectPos.y;  // Stack from the bottom

    updateEffectsDisplay();
    XuiSetTimer(m_hObj, INVENTORY_UPDATE_EFFECTS_TIMER_ID,
...
HRESULT CXuiSceneInventory::handleCustomTimer(XUIMessageTimer* pTimer,
                                              BOOL& bHandled) {
...
void CXuiSceneInventory::InitDataAssociations(int iPad,
    ...
    CXuiSceneAbstractContainer::InitDataAssociations(
        iPad, menu, InventoryMenu::INV_SLOT_START);
}
...
void CXuiSceneInventory::updateEffectsDisplay() {
    ...
    float fHeight = (effectCount * m_effectDisplaySpacing) -
                    (m_effectDisplaySpacing - m_effectDisplayHeight);
    ...
    for (unsigned int i = 0; i < MAX_EFFECTS; ++i) {
        if (it != activeEffects->end()) {
            ...
            if (i > 0)
                position.y -= fNextEffectYOffset;  // Stack from the bottom
