...
HRESULT CXuiSceneAnvil::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    CXuiSceneAbstractContainer::Initialize(
        m_iPad, m_repairMenu, true, RepairMenu::INV_SLOT_START,
...
HRESULT CXuiSceneAnvil::OnNotifyValueChanged(
    ...
    if (hObjSource == m_editName) {
        ...
        // 4J-PB - This was stopping the player deleting the last character in
        // the name using the chatpad
        // if(!newValue.empty() && !(newValue.size() == 1 && newValue.at(0) ==
        // L' '))
        {
...
HRESULT CXuiSceneAnvil::handleCustomTimer(XUIMessageTimer* pTimer,
                                          BOOL& bHandled) {
...
void CXuiSceneAnvil::InitDataAssociations(int iPad, AbstractContainerMenu* menu,
                                          int startIndex /*= 0*/) {
    ...
    m_ingredient2Control->SetData(iPad, menu, 1, 1,
                                  RepairMenu::ADDITIONAL_SLOT);
    ...
    CXuiSceneAbstractContainer::InitDataAssociations(
        iPad, menu, RepairMenu::INV_SLOT_START);
}

void CXuiSceneAnvil::handleEditNamePressed() {
    // 4J Stu - The edit control already handles A/Start presses, so ignore
    // anything else
