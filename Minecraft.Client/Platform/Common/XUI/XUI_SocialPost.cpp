...
HRESULT CScene_SocialPost::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    XuiControlSetText(m_LabelDescription,
                      app.GetString(IDS_SOCIAL_LABEL_DESCRIPTION));
    XuiControlSetText(m_EditDesc,
                      app.GetString(IDS_SOCIAL_DEFAULT_DESCRIPTION));
    ...
    bool bHasAllText = /*( wTitle.length()!=0) && */ (wCaption.length() != 0) &&
                       (wDesc.length() != 0);
...
HRESULT CScene_SocialPost::OnNotifyValueChanged(
    HXUIOBJ hObjSource, XUINotifyValueChanged* pValueChangedData,
    BOOL& rfHandled) {
    std::wstring wCaption = m_EditCaption.GetText();
    std::wstring wDesc = m_EditDesc.GetText();

    if (/*(hObjSource == m_EditTitle) || */ (hObjSource == m_EditCaption) ||
        (hObjSource == m_EditDesc)) {
        ...
        bool bHasAllText =
            /*( wTitle.length()!=0) &&*/ (wCaption.length() != 0) &&
            (wDesc.length() != 0);
        m_OK.SetEnable(bHasAllText);
    }
...
HRESULT CScene_SocialPost::OnControlNavigate(
...
//----------------------------------------------------------------------------------
HRESULT CScene_SocialPost::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    if (hObjPressed == m_OK) {
        CSocialManager::Instance()->SetSocialPostText(
            m_wTitle.c_str(), m_EditCaption.GetText(), m_EditDesc.GetText());

        CSocialManager::Instance()->PostImageToSocialNetwork(
            eFacebook, pNotifyPressData->UserIndex, false);
...
HRESULT CScene_SocialPost::OnKeyDown(XUIMessageInput* pInputData,
                                     BOOL& rfHandled) {
...
HRESULT CScene_SocialPost::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                             BOOL& bHandled) {
    bHandled = true;
