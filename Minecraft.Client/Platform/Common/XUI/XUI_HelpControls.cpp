...
HRESULT CScene_Controls::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    swprintf(layoutString, 128, L"%ls%ls", VER_PRODUCTVERSION_STR_W,
             app.DLCInstallProcessCompleted() ? L"_DLC" : L" ");
    ...
    bool bInvertLook =
        app.GetGameSettings(m_iPad, eGameSetting_ControlInvertLook) != 0;
    m_InvertLook.SetCheck(bInvertLook);

    bool bSouthPaw =
        app.GetGameSettings(m_iPad, eGameSetting_ControlSouthPaw) != 0;
    m_SouthPaw.SetCheck(bSouthPaw);

    return S_OK;
}

HRESULT CScene_Controls::OnTransitionStart(XUIMessageTransition* pTransition,
                                           BOOL& bHandled) {
...
HRESULT CScene_Controls::OnKeyDown(XUIMessageInput* pInputData,
                                   BOOL& rfHandled) {
...
void CScene_Controls::PositionText(int iPad, int iTextID,
    ...
    // get the visual of the fig group and use any scaling in it to adjust the
    // text positions
    ...
    if (m_TextPresenterA[m_iCurrentTextIndex] == NULL) {
        HXUIOBJ hObj = NULL;
        HRESULT hr =
            XuiControlGetVisual(m_TextA[m_iCurrentTextIndex].m_hObj, &hObj);
        hr = XuiElementGetChildById(hObj, L"Text",
                                    &m_TextPresenterA[m_iCurrentTextIndex]);
    }

    pwchText = app.GetString(iTextID);
    HRESULT hr = XuiTextPresenterMeasureText(
        m_TextPresenterA[m_iCurrentTextIndex], pwchText, &xuiRect);
    ...
    if (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen()) {
        ...
        vpos.y = (controlDetailsA[iControlDetailsIndex].vPos.y * vScale.y) -
                 (xuiRect.bottom / 2.0f);
    } else {
        if (controlDetailsA[iControlDetailsIndex].iAlign == ALIGN_END) {
            vpos.x = (controlDetailsA[iControlDetailsIndex].vPos.x + 5.0f);
        ...
        vpos.y = (controlDetailsA[iControlDetailsIndex].vPos.y) -
                 (xuiRect.bottom / 2.0f);
    }
...
void CScene_Controls::PositionTextDirect(int iPad, int iTextID,
                                         int iControlDetailsIndex, bool bShow) {
    // position the text depending on the control id
    // int iTextWidth;
    XUIRect xuiRect;
    LPCWSTR pwchText;
...
HRESULT CScene_Controls::OnNotifySelChanged(
...
HRESULT CScene_Controls::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    if (hObjPressed == m_InvertLook.m_hObj) {
        ...
        app.SetGameSettings(m_iPad, eGameSetting_ControlScheme,
                            (unsigned char)m_SchemeList.GetCurSel());
        LPWSTR layoutString = new wchar_t[128];
        swprintf(layoutString, 128, L"%ls : %ls",
                 app.GetString(IDS_CURRENT_LAYOUT),
                 app.GetString(m_iSchemeTextA[m_SchemeList.GetCurSel()]));
...
HRESULT CScene_Controls::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                           BOOL& bHandled) {
    bHandled = true;
    return app.AdjustSplitscreenScene_PlayerChanged(m_hObj, &m_OriginalPosition,
                                                    m_iPad, bJoining, 32.0f);
}
