...
//----------------------------------------------------------------------------------
HRESULT CScene_TutorialPopup::OnInit(XUIMessageInit* pInitData,
                                     BOOL& bHandled) {
    HRESULT hr = S_OK;
...
HRESULT CScene_TutorialPopup::OnDestroy() { return S_OK; }
...
void CScene_TutorialPopup::UpdateInteractScenePosition(bool visible) {
    ...
    HXUICLASS creativeInventoryClass =
        XuiFindClass(L"CXuiSceneInventoryCreative");
    ...
    bool isCraftingScene =
        XuiClassDerivesFrom(sceneClass, craftingClass) != FALSE;
    bool isCreativeScene =
        XuiClassDerivesFrom(sceneClass, creativeInventoryClass) != FALSE;
    ...
    if (bAllowAnim) {
        ...
        if (XuiClassDerivesFrom(sceneClass, inventoryClass) ||
            ...
            XuiClassDerivesFrom(sceneClass, tradingClass) || isCreativeScene ||
            isCraftingScene) {
...
HRESULT CScene_TutorialPopup::_SetDescription(CXuiScene* interactScene,
                                              LPCWSTR desc, LPCWSTR title,
                                              bool allowFade, bool isReminder) {
    HRESULT hr = S_OK;
    ...
    if (desc == NULL) {
        ...
        if (allowFade) {
            ...
            XuiSetTimer(m_hObj, TUTORIAL_POPUP_FADE_TIMER_ID,
                        tutorial->GetTutorialDisplayMessageTime());
        } else {
            XuiKillTimer(m_hObj, TUTORIAL_POPUP_FADE_TIMER_ID);
        }
        ...
        if (isReminder) {
            ...
            swprintf(formatting, 40, L"<font color=\"#%08x\" size=\"%d\">",
                     app.GetHTMLColour(eHTMLColor_White), m_textFontSize);
        ...
        if (titleShowAtStart != titleShowAtEnd) {
            ...
            // 4J Stu - For some reason when we resize the scene it resets the
            // size of the HTML control We don't want that to happen, so get
            // it's size before and set it back after
        ...
        m_description.SetBounds(fWidth,
                                (float)(contentDims.nPageHeight + heightDiff));
    }
    return hr;
}

HRESULT CScene_TutorialPopup::SetDescription(int iPad,
    ...
    if (XuiClassDerivesFrom(objClass, thisClass)) {
        ...
        std::wstring parsed = pThis->_SetIcon(info->icon, info->iAuxVal,
                                              info->isFoil, info->desc);
...
std::wstring CScene_TutorialPopup::_SetIcon(int icon, int iAuxVal, bool isFoil,
                                            LPCWSTR desc) {
    ...
    if (icon != TUTORIAL_NO_ICON) {
        ...
        m_pCraftingPic->SetIcon(m_iPad, icon, iAuxVal, 1, 10, 31, false,
                                itemIsFoil);
    } else {
        ...
        if (iconTagStartPos > 0 && iconStartPos < (int)temp.length()) {
            ...
            if (iconEndPos > iconStartPos && iconEndPos < (int)temp.length()) {
                ...
                bool itemIsFoil = false;
                itemIsFoil = (std::shared_ptr<ItemInstance>(
                                  new ItemInstance(iconId, 1, iAuxVal)))
                                 ->isFoil();
                if (!itemIsFoil) itemIsFoil = isFoil;

                m_pCraftingPic->SetIcon(m_iPad, iconId, iAuxVal, 1, 10, 31,
                                        false, itemIsFoil);
        ...
        // remove any icon text
            ...
            m_pCraftingPic->SetIcon(m_iPad, Tile::workBench->id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*SticksIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::stick->id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*PlanksIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Tile::wood->id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*WoodenShovelIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::shovel_wood->id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*WoodenHatchetIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::hatchet_wood->id, 0, 1, 10,
                                    31, false);
        } else if (temp.find(L"{*WoodenPickaxeIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::pickAxe_wood->id, 0, 1, 10,
                                    31, false);
        } else if (temp.find(L"{*FurnaceIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Tile::furnace_Id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*WoodenDoorIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::door_wood->id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*TorchIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Tile::torch_Id, 0, 1, 10, 31,
                                    false);
            ...
            m_pCraftingPic->SetIcon(m_iPad, Item::fishingRod_Id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*FishIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::fish_raw_Id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*MinecartIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, Item::minecart_Id, 0, 1, 10, 31,
                                    false);
            ...
            m_pCraftingPic->SetIcon(m_iPad, Tile::goldenRail_Id, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*StructuresIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, XZP_ICON_STRUCTURES, 0, 1, 10, 31,
                                    false);
        } else if (temp.find(L"{*ToolsIcon*}") != std::wstring::npos) {
            m_pCraftingPic->SetIcon(m_iPad, XZP_ICON_TOOLS, 0, 1, 10, 31,
                                    false);
    ...
    if (iconShowAtStart != iconShowAtEnd) {
        ...
        // 4J Stu - For some reason when we resize the scene it resets the size
        // of the HTML control We don't want that to happen, so get it's size
...
std::wstring CScene_TutorialPopup::_SetImage(std::wstring& desc) {
    ...
    if (imageTagStartPos > 0 && imageStartPos < (int)desc.length()) {
        ...
        if (imageEndPos > imageStartPos && imageEndPos < (int)desc.length()) {
            ...
            desc.replace(imageTagStartPos,
                         imageEndPos - imageTagStartPos + closeTag.length(),
                         L"");
        }
    } else {
        // hide the icon slot
...
std::wstring CScene_TutorialPopup::ParseDescription(int iPad,
                                                    std::wstring& text) {
    ...
    if (app.GetLocalPlayerCount() > 1) {
        // TODO TU-1 - This should really be a string as well rather than
        // hardcoded here
        text = replaceAll(
            text, L"{*EXIT_PICTURE*}",
            L"<div align=\"center\"><img height=\"90\" width=\"160\" "
            L"src=\"TutorialExitScreenshot.png\"/></div>");

        // TODO TU-1 - This should also be separate strings, or move these
        // things out of a localisable file so we can add/change them at will
        text = replaceAll(text, L"height=\"30\" width=\"30\"",
                          L"height=\"20\" width=\"20\"");
    } else {
        text = replaceAll(text, L"{*EXIT_PICTURE*}",
                          app.GetString(IDS_TUTORIAL_HTML_EXIT_PICTURE));
    }
    /*
...
HRESULT CScene_TutorialPopup::OnCustomMessage_Splitscreenplayer(
