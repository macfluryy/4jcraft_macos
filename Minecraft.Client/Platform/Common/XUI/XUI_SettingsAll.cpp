...
HRESULT CScene_SettingsAll::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    XuiControlSetText(m_Buttons[BUTTON_ALL_OPTIONS],
                      app.GetString(IDS_OPTIONS));
    XuiControlSetText(m_Buttons[BUTTON_ALL_AUDIO], app.GetString(IDS_AUDIO));
    XuiControlSetText(m_Buttons[BUTTON_ALL_CONTROL],
                      app.GetString(IDS_CONTROL));
    XuiControlSetText(m_Buttons[BUTTON_ALL_GRAPHICS],
                      app.GetString(IDS_GRAPHICS));
    XuiControlSetText(m_Buttons[BUTTON_ALL_UI],
                      app.GetString(IDS_USER_INTERFACE));
    XuiControlSetText(m_Buttons[BUTTON_ALL_RESETTODEFAULTS],
                      app.GetString(IDS_RESET_TO_DEFAULTS));
    ...
    if (ProfileManager.GetPrimaryPad() != m_iPad) {
        ...
        m_Buttons[BUTTON_ALL_RESETTODEFAULTS].GetBounds(&fButtonWidth,
                                                        &fButtonHeight);
...
HRESULT CScene_SettingsAll::OnKeyDown(XUIMessageInput* pInputData,
                                      BOOL& rfHandled) {
...
HRESULT CScene_SettingsAll::OnControlNavigate(
...
HRESULT CScene_SettingsAll::OnTransitionStart(XUIMessageTransition* pTransition,
                                              BOOL& bHandled) {
    // HRESULT hr;
...
//----------------------------------------------------------------------------------
HRESULT CScene_SettingsAll::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    while ((uiButtonCounter < BUTTONS_ALL_MAX) &&
    ...
    switch (uiButtonCounter) {
        ...
        case BUTTON_ALL_RESETTODEFAULTS: {
            ...
            StorageManager.RequestMessageBox(
                IDS_DEFAULTS_TITLE, IDS_DEFAULTS_TEXT, uiIDA, 2,
                pNotifyPressData->UserIndex,
                &CScene_SettingsAll::ResetDefaultsDialogReturned, this,
                app.GetStringTable());
        } break;
    }
...
HRESULT CScene_SettingsAll::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                              BOOL& bHandled) {
    bHandled = true;
...
int CScene_SettingsAll::ResetDefaultsDialogReturned(
    void* pParam, int iPad, C4JStorage::EMessageResult result) {
    // CScene_SettingsAll* pClass = (CScene_SettingsAll*)pParam;

    // results switched for this dialog
    if (result == C4JStorage::EMessage_ResultDecline) {
        app.SetDefaultOptions(ProfileManager.GetDashboardProfileSettings(iPad),
                              iPad);
        // if the profile data has been changed, then force a profile write
