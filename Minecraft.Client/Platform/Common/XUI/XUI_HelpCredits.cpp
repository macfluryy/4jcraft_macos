...
SCreditTextItemDef CScene_Credits::gs_aCreditDefs[MAX_CREDIT_STRINGS] = {
    ...
    {L"Microsoft Studios", NO_TRANSLATED_STRING, NO_TRANSLATED_STRING,
     eExtraLargeText},
    {L"", NO_TRANSLATED_STRING, NO_TRANSLATED_STRING,
     eSmallText},  // extra blank line
    {L"", NO_TRANSLATED_STRING, NO_TRANSLATED_STRING,
...
// Table tells us how many text elements of each type are available in the
// scene.
...
HRESULT CScene_Credits::OnInit(XUIMessageInit* pInitData, BOOL& bHandled) {
    ...
    if (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen()) {
        CREDITS_SCREEN_MIN_Y =
            200.0f /
            1.5f;  // Y pos at which credits are removed from top of screen.
        CREDITS_SCREEN_MAX_Y =
            630.0f /
            1.5f;  // Y pos at which credits appear at bottom of screen.
        CREDITS_FADE_HEIGHT =
            100.0f / 1.5f;  // Height over which credits fade in or fade out.
        ...
        CREDITS_SCREEN_MAX_Y =
            630.0f;  // Y pos at which credits appear at bottom of screen.
        CREDITS_FADE_HEIGHT =
            100.0f;  // Height over which credits fade in or fade out.
    ...
    for (int i = 0; i < eNumTextTypes; ++i) {
        ...
        m_aTextTypes[i].m_appTextElements =
            new CXuiControl*[m_aTextTypes[i].m_iMaxElements];
    ...
    m_iNumTextDefs = MAX_CREDIT_STRINGS;  // sizeof( gs_aCreditDefs ) / sizeof(
                                          // SCreditTextItemDef );
...
HRESULT CScene_Credits::OnKeyDown(XUIMessageInput* pInputData,
                                  BOOL& rfHandled) {
...
HRESULT CScene_Credits::OnControlNavigate(
...
//----------------------------------------------------------------------------------
HRESULT CScene_Credits::OnNotifyPressEx(HXUIOBJ hObjPressed,
...
HRESULT CScene_Credits::OnTimer(XUIMessageTimer* pTimer, BOOL& bHandled) {
    ...
    if (pTimer->nId == CREDITS_TICK_TIMER_ID) {
        ...
        if ((m_iCurrDefIndex == -1) ||
            (m_fMoveSinceLastDef >= m_fMoveToNextDef)) {
            ...
            if (pTextType->m_iNumUsedElements < pTextType->m_iMaxElements) {
                ...
                CXuiControl* pElement =
                    pTextType->m_appTextElements[pTextType->m_iNextFreeElement];
                ...
                if (pTextType->m_iNextFreeElement >=
                    pTextType->m_iMaxElements) {
                ...
                if (iNextDef == m_iNumTextDefs) {
                    ...
                    m_fMoveToNextDef =
                        gs_aLineSpace[gs_aCreditDefs[iNextDef].m_eType];
                } else {
                    // Determine space to next item.
                    m_fMoveToNextDef =
                        gs_aLineSpace[app.GetDLCCredits(iNextDef -
                                                        MAX_CREDIT_STRINGS)
                                          ->m_eType];
                }
        ...
        for (int i = 0; i < eNumTextTypes; ++i) {
            ...
            // Process in reverse order from latest one back (so that we can
            // easily remove oldest if it has scrolled off the top).
            ...
            for (int j = 0; j < pTextType->m_iNumUsedElements; ++j) {
                ...
                CXuiControl* pElement =
                    pTextType->m_appTextElements[iElementIndex];
                ...
                if (vTextPos.y < CREDITS_SCREEN_MIN_Y) {
                    ...
                    float fOpacity = 1.0f;
                    if (vTextPos.y <
                        (CREDITS_SCREEN_MIN_Y + CREDITS_FADE_HEIGHT)) {
                        fOpacity = (vTextPos.y - CREDITS_SCREEN_MIN_Y) /
                                   CREDITS_FADE_HEIGHT;
                    } else if (vTextPos.y >
                               (CREDITS_SCREEN_MAX_Y - CREDITS_FADE_HEIGHT)) {
                        fOpacity = (CREDITS_SCREEN_MAX_Y - vTextPos.y) /
                                   CREDITS_FADE_HEIGHT;
                    }
