...
//----------------------------------------------------------------------------------
HRESULT CScene_MultiGameInfo::OnInit(XUIMessageInit* pInitData,
                                     BOOL& bHandled) {
    MapChildControls();
    ...
    for (unsigned int i = 0; i < MINECRAFT_NET_MAX_PLAYERS; ++i) {
        if (m_selectedSession->data.players[i] != NULL) {
            ...
            {
                playersList.SetText(
                    i, convStringToWstring(m_selectedSession->data.szPlayers[i])
                           .c_str());
            }
        } else {
    ...
    switch (
        app.GetGameHostOption(uiGameHostSettings, eGameHostOption_Difficulty)) {
    ...
    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_Gamertags))
        m_gamertagsOn.SetText(app.GetString(IDS_ON));
    else
        m_gamertagsOn.SetText(app.GetString(IDS_OFF));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_Structures))
        m_structuresOn.SetText(app.GetString(IDS_ON));
    else
        m_structuresOn.SetText(app.GetString(IDS_OFF));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_LevelType))
        m_levelType.SetText(app.GetString(IDS_LEVELTYPE_SUPERFLAT));
    else
        m_levelType.SetText(app.GetString(IDS_LEVELTYPE_NORMAL));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_PvP))
        m_pvpOn.SetText(app.GetString(IDS_ON));
    else
        m_pvpOn.SetText(app.GetString(IDS_OFF));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_TrustPlayers))
        m_trustPlayers.SetText(app.GetString(IDS_ON));
    else
        m_trustPlayers.SetText(app.GetString(IDS_OFF));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_TNT))
        m_tntOn.SetText(app.GetString(IDS_ON));
    else
        m_tntOn.SetText(app.GetString(IDS_OFF));

    if (app.GetGameHostOption(uiGameHostSettings, eGameHostOption_FireSpreads))
        m_fireOn.SetText(app.GetString(IDS_ON));
    else
        m_fireOn.SetText(app.GetString(IDS_OFF));
...
//----------------------------------------------------------------------------------
HRESULT CScene_MultiGameInfo::OnNotifyPressEx(HXUIOBJ hObjPressed,
    ...
    if (hObjPressed == m_JoinGame) {
        ...
        if (connectedControllers == 1 || !RenderManager.IsHiDef()) {
            ...
            ProfileManager.RequestSignInUI(
                ...
                &CScene_MultiGameInfo::StartGame_SignInReturned, this,
...
HRESULT CScene_MultiGameInfo::OnKeyDown(XUIMessageInput* pInputData,
                                        BOOL& rfHandled) {
    if (m_bIgnoreInput) return S_OK;
...
HRESULT CScene_MultiGameInfo::OnNotifySelChanged(
...
HRESULT CScene_MultiGameInfo::OnNotifySetFocus(HXUIOBJ hObjSource,
                                               XUINotifyFocus* pNotifyFocusData,
                                               BOOL& bHandled) {
    if (playersList.TreeHasFocus() && playersList.GetItemCount() > 0) {
        ui.SetTooltips(DEFAULT_XUI_MENU_USER, -1, IDS_TOOLTIPS_BACK, -1,
                       IDS_TOOLTIPS_VIEW_GAMERCARD);
        bHandled = TRUE;
    }
    return S_OK;
}

HRESULT CScene_MultiGameInfo::OnNotifyKillFocus(
...
int CScene_MultiGameInfo::StartGame_SignInReturned(void* pParam, bool bContinue,
                                                   int iPad) {
