...
void GetRenderAndSamplerStates(IDirect3DDevice9* pDevice, DWORD* RenderStateA,
                               DWORD* SamplerStateA);
void SetRenderAndSamplerStates(IDirect3DDevice9* pDevice, DWORD* RenderStateA,
                               DWORD* SamplerStateA);

// 4J-PB - Done - #pragma message(__LOC__"Theme should be GREEN SIGNED before
// _FINAL_BUILD")
...
D3DRENDERSTATETYPE RenderStateModes[] = {
    D3DRS_FILLMODE,       D3DRS_CULLMODE,        D3DRS_ALPHABLENDENABLE,
    D3DRS_VIEWPORTENABLE, D3DRS_BLENDOP,         D3DRS_SRCBLEND,
    D3DRS_DESTBLEND,      D3DRS_COLORWRITEENABLE};

D3DSAMPLERSTATETYPE SamplerStateModes[] = {D3DSAMP_MINFILTER, D3DSAMP_MAGFILTER,
...
HRESULT InitD3D(IDirect3DDevice9** ppDevice, D3DPRESENT_PARAMETERS* pd3dPP) {
    IDirect3D9* pD3D;

    pD3D = Direct3DCreate9(D3D_SDK_VERSION);

    // Set up the structure used to create the D3DDevice
    // Using a permanent 1280x720 backbuffer now no matter what the actual video
    // resolution.right Have also disabled letterboxing, which would letterbox a
    // 1280x720 output if it detected a 4:3 video source - we're doing an
    // anamorphic squash in this mode so don't need this functionality.
    ...
    // pd3dPP->Flags				   =
    // D3DPRESENTFLAG_NO_LETTERBOX; ERR[D3D]: Can't set
    // D3DPRESENTFLAG_NO_LETTERBOX when wide-screen is enabled 	in the
    // launcher/dashboard.
    if (g_bWidescreen)
        pd3dPP->Flags = 0;
    else
    ...
    return pD3D->CreateDevice(
        ...
        D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_BUFFER_2_FRAMES, pd3dPP,
        ppDevice);
}
...
int __cdecl main() {
    ...
    // XUI render can change the following renderstates, so store then before
    // and re-set them after the xui render
    ...
    if (FAILED(hr)) {
        app.DebugPrintf("Failed initializing D3D.\n");
        return -1;
    }

    // Initialize the application, assuming sharing of the d3d interface.
    hr = app.InitShared(pDevice, &d3dpp, XuiPNGTextureLoader);

    if (FAILED(hr)) {
        app.DebugPrintf("Failed initializing application.\n");
    ...
    // 4J Stu - XACT was creating these automatically, but we need them for
    // QNet. The setup params are just copied from a sample app and may need
    // changed for our purposes Start XAudio2
    ...
    // Create an XAudio2 mastering voice (utilized by XHV2 when voice data is
    // mixed to main speakers)
    hr = g_pXAudio2->CreateMasteringVoice(
        &g_pXAudio2MasteringVoice, XAUDIO2_DEFAULT_CHANNELS,
        XAUDIO2_DEFAULT_SAMPLERATE, 0, 0, NULL);
    if (FAILED(hr)) {
        app.DebugPrintf(
            "Creating XAudio2 mastering voice failed (err = 0x%08x)!\n", hr);
        app.FatalLoadError();
    }
    ...
    ProfileManager.RegisterAward(eAward_mine100Blocks, GAMER_PICTURE_GAMERPIC1,
                                 eAwardType_GamerPic, false,
                                 app.GetStringTable(), IDS_AWARD_TITLE,
                                 IDS_AWARD_GAMERPIC1, IDS_CONFIRM_OK);
    ProfileManager.RegisterAward(eAward_kill10Creepers, GAMER_PICTURE_GAMERPIC2,
                                 eAwardType_GamerPic, false,
                                 app.GetStringTable(), IDS_AWARD_TITLE,
                                 IDS_AWARD_GAMERPIC2, IDS_CONFIRM_OK);

    ProfileManager.RegisterAward(
        eAward_eatPorkChop, AVATARASSETAWARD_PORKCHOP_TSHIRT,
        eAwardType_AvatarItem, false, app.GetStringTable(), IDS_AWARD_TITLE,
        IDS_AWARD_AVATAR1, IDS_CONFIRM_OK);
    ProfileManager.RegisterAward(eAward_play100Days, AVATARASSETAWARD_WATCH,
                                 eAwardType_AvatarItem, false,
                                 app.GetStringTable(), IDS_AWARD_TITLE,
                                 IDS_AWARD_AVATAR2, IDS_CONFIRM_OK);
    ProfileManager.RegisterAward(eAward_arrowKillCreeper, AVATARASSETAWARD_CAP,
                                 eAwardType_AvatarItem, false,
                                 app.GetStringTable(), IDS_AWARD_TITLE,
                                 IDS_AWARD_AVATAR3, IDS_CONFIRM_OK);

    ProfileManager.RegisterAward(eAward_socialPost, 0, eAwardType_Theme, false,
                                 app.GetStringTable(), IDS_AWARD_TITLE,
                                 IDS_AWARD_THEME, IDS_CONFIRM_OK, THEME_NAME,
                                 THEME_FILESIZE);
    ...
    StorageManager.Init(
        app.GetString(IDS_DEFAULT_SAVENAME), "savegame.dat", FIFTY_ONE_MB,
        &CConsoleMinecraftApp::DisplaySavingMessage, (LPVOID)&app, GROUP_ID);
    ...
    // set a function to be called when the ethernet is disconnected, so we can
    // back out if required
    ProfileManager.SetNotificationsCallback(
        &CConsoleMinecraftApp::NotificationsCallback, (LPVOID)&app);
    ...
    // versions
    ProfileManager.SetOldProfileVersionCallback(
        &CConsoleMinecraftApp::OldProfileVersionCallback, (LPVOID)&app);

    // Set a callback for when there is a read error on profile data
    ProfileManager.SetProfileReadErrorCallback(
        &CConsoleMinecraftApp::ProfileReadErrorCallback, (LPVOID)&app);
    ...
    StorageManager.RegisterMarketplaceCountsCallback(
        &CConsoleMinecraftApp::MarketplaceCountsCallback, (LPVOID)&app);
    // Kinect !

    // 	if(XNuiGetHardwareStatus()!=0)
    // 	{
    // 		// If the Kinect Sensor is not physically connected, this
    // function returns 0.
    // 		NuiInitialize(NUI_INITIALIZE_FLAG_USES_HIGH_QUALITY_COLOR |
    // NUI_INITIALIZE_FLAG_USES_DEPTH |
    // 			NUI_INITIALIZE_FLAG_EXTRAPOLATE_FLOOR_PLANE |
    // NUI_INITIALIZE_FLAG_USES_FITNESS | NUI_INITIALIZE_FLAG_NUI_GUIDE_DISABLED
    // |
    // NUI_INITIALIZE_FLAG_SUPPRESS_AUTOMATIC_UI,NUI_INITIALIZE_DEFAULT_HARDWARE_THREAD
    // );
    // 	}
    ...
    // set the achievement text for a trial achievement, now we have the string
    // table loaded
    ...
    ProfileManager.SetTrialAwardText(eAwardType_GamerPic, IDS_UNLOCK_TITLE,
                                     IDS_UNLOCK_GAMERPIC_TEXT);
    ProfileManager.SetTrialAwardText(eAwardType_AvatarItem, IDS_UNLOCK_TITLE,
                                     IDS_UNLOCK_AVATAR_TEXT);
...
#pragma message(__LOC__ "Need to define the _FINAL_BUILD before submission")
    ...
    // 4J-PB - Having to add in code to stop people being able to play a build
    // from Partnernet unless they know the code
...
// 		app.RunFrame();
//
// 		// 4J : WESTY : Added to ensure we always have clear background
// for intro. 		RenderManager.SetClearColour(D3DCOLOR_RGBA(0,0,0,255));
...
// 	}
    ...
    while (app.IntroRunning()) {
        ...
        // 4J : WESTY : Added to ensure we always have clear background for
        // intro.
    ...
    while (TRUE) {
        ...
        // store the minecraft renderstates, and re-set them after the xui
        // render
        ...
        for (int i = 0; i < 5; i++) {
            if (SamplerStateA2[i] != SamplerStateA[i]) {
                ...
                pDevice->SetSamplerState(0, SamplerStateModes[i],
                                         SamplerStateA[i]);
            }
        }
        ...
        if (app.uiGameDefinedDataChangedBitmask != 0) {
            ...
            for (int i = 0; i < XUSER_MAX_COUNT; i++) {
                if (app.uiGameDefinedDataChangedBitmask & (1 << i)) {
                    ...
                    // Update the selected item position based on the user's UI
                    // setting
