...
#define GDRAW_GL_EXTENSION_LIST                                                \
    ...
    GLE(CompressedTexImage2D, "CompressedTexImage2DARB",                       \
...
#define GLE(id, import, procname) \
    gl##id = (PFNGL##procname##PROC)wglGetProcAddress("gl" import);
    GDRAW_GL_EXTENSION_LIST
#undef GLE
}
...
GDrawFunctions* gdraw_GL_CreateContext(S32 w, S32 h, S32 msaa_samples) {
    ...
    if (s == NULL) {
        assert(s != NULL);  // if this is NULL, you're probably trying to create
                            // the device too early
        return NULL;
    }
    ...
    if (!hasext(s, "GL_ARB_multitexture") ||
        ...
        !hasext(
            s,
            "GL_ARB_texture_non_power_of_two") ||  // with caveats - see below!
    ...
    // we require ARB_texture_non_power_of_two - on actual HW, this may either
    // give us "full" non-power-of-two support, or "conditional"
    // non-power-of-two (wrap mode must be CLAMP_TO_EDGE, no mipmaps). figure
    // out which it is using this heuristic by Unity's Aras Pranckevicius
    // (thanks!):
    //   http://www.aras-p.info/blog/2012/10/17/non-power-of-two-textures/
    //
    // we use the second heuristic (texture size <8192 for cards without full
    // NPOT support) since we don't otherwise use ARB_fragment_program and don't
    // want to create a program just to be able to query
