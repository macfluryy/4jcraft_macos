...
IDOC extern int gdraw_D3D_SetResourceLimits(gdraw_d3d_resourcetype type,
...
IDOC extern GDrawFunctions* gdraw_D3D_CreateContext(IDirect3DDevice9* dev,
                                                    S32 w, S32 h);
/* Creates a GDraw context for rendering using D3D. You need to pass in the D3D
   device and the width/height of the content you're displaying.
...
/* Reset the size used for internal rendertargets defined by CreateContext.
   Flushes all existing rendertargets if the size changes. */

IDOC extern void gdraw_D3D_DestroyContext(void);
/* Destroys the current GDraw context, if any. */

IDOC extern void gdraw_D3D_SetTileOrigin(IDirect3DSurface9* rt,
                                         IDirect3DSurface9* depth, S32 x,
                                         S32 y);
/* This sets the main rendertarget that GDraw should render to, the
   corresponding depth/stencil surface, and the x/y position where to draw the
   top-left of the current tile (to be used for tiled rendering). You need to
   call this before Iggy calls any rendering functions. */
...
gdraw_D3D_BeginCustomDraw(IggyCustomDrawCallbackRegion* Region, D3DMATRIX* mat);
...
IDOC extern void RADLINK
gdraw_D3D_EndCustomDraw(IggyCustomDrawCallbackRegion* Region);
/* Call at the end of Iggy custom draw callback so GDraw can restore its render
 * states. */

IDOC extern void RADLINK gdraw_D3D_GetResourceUsageStats(
    gdraw_d3d_resourcetype type, S32* handles_used, S32* bytes_used);
/* D3D only: Get resource usage stats for last frame.
   ...
   GDraw during the last frame. Caveat: This counts the number of bytes that
   GDraw knows about. 3D hardware usually has its own management overhead,
   alignment requirements, allocation granularity and so on. In short, this is
   not an accurate estimate of how much memory is actually used by the GPU - it
   is a lower bound, though, and makes for a useful ballpark estimate. */

IDOC extern GDrawTexture* gdraw_D3D_WrappedTextureCreate(
    IDirect3DTexture9* tex_handle);
...
IDOC extern void gdraw_D3D_WrappedTextureChange(GDrawTexture* tex,
                                                IDirect3DTexture9* tex_handle);
...
extern GDrawTexture* RADLINK gdraw_D3D_MakeTextureFromResource(
