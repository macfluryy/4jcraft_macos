...
typedef enum gdraw_d3d10_resourcetype {
    ...
    GDRAW_D3D10_RESOURCE_dynbuffer,  // Streaming buffer for dynamic
                                     // vertex/index data (handle count ignored)

    GDRAW_D3D10_RESOURCE__count,
} gdraw_d3d10_resourcetype;

IDOC extern int gdraw_D3D10_SetResourceLimits(gdraw_d3d10_resourcetype type,
...
IDOC extern GDrawFunctions* gdraw_D3D10_CreateContext(ID3D10Device* dev, S32 w,
                                                      S32 h);
/* Creates a GDraw context for rendering using D3D. You need to pass in the D3D
   device and the width/height of render target textures.
...
/* Changes the current render target size (and recreates all rendertargets if
   ...
   size is smaller than it was previously. As with $gdraw_D3D10_CreateContext,
   the width and height specified here are only minimums; GDraw will reallocate
   larger rendertargets as needed. */

IDOC extern void gdraw_D3D10_SetTileOrigin(
    ID3D10RenderTargetView* main_rt, ID3D10DepthStencilView* main_ds,
...
IDOC extern void RADLINK gdraw_D3D10_BeginCustomDraw(
...
gdraw_D3D10_EndCustomDraw(IggyCustomDrawCallbackRegion* Region);
/* Call at the end of Iggy custom draw callback so GDraw can restore its render
 * states. */

IDOC extern void RADLINK gdraw_D3D10_GetResourceUsageStats(
    gdraw_d3d10_resourcetype type, S32* handles_used, S32* bytes_used);
...
IDOC extern GDrawTexture* gdraw_D3D10_WrappedTextureCreate(
    ID3D10ShaderResourceView* tex_view);
...
IDOC extern void gdraw_D3D10_WrappedTextureChange(
    GDrawTexture* tex, ID3D10ShaderResourceView* tex_view);
...
extern GDrawTexture* RADLINK gdraw_D3D10_MakeTextureFromResource(
