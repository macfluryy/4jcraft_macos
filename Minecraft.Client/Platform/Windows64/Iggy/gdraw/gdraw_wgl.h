...
extern void gdraw_GL_SetTileOrigin(
    S32 vx, S32 vy,
    U32 framebuffer);  // framebuffer=FBO handle, or 0 for main frame buffer
...
extern void gdraw_GL_WrappedTextureChange(GDrawTexture* tex,
                                          ...
                                          rrbool new_has_mipmaps);
extern void gdraw_GL_WrappedTextureDestroy(GDrawTexture* tex);

extern void gdraw_GL_BeginCustomDraw(
    struct IggyCustomDrawCallbackRegion* region, F32* matrix);
extern void gdraw_GL_EndCustomDraw(struct IggyCustomDrawCallbackRegion* region);

extern GDrawTexture* RADLINK gdraw_GL_MakeTextureFromResource(
    U8* resource_file, S32 resource_len, IggyFileTextureRaw* texture);
