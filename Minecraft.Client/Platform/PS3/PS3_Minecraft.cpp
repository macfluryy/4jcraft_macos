...
char secureFileId[CELL_SAVEDATA_SECUREFILEID_SIZE] = {
    0xEE, 0xA9, 0x37, 0xCC, 0x5B, 0xD4, 0xD9, 0x0D,
    0x55, 0xED, 0x25, 0x31, 0xFA, 0x33, 0xBD, 0xC4};
...
//--------------------------------------------------------------------------------------
HRESULT InitDevice() { return S_OK; }
...
int simpleMessageBoxCallback(UINT uiTitle, UINT uiText, UINT* uiOptionA,
                             ...
                             LPVOID lpParam) {
    ui.RequestMessageBox(uiTitle, uiText, uiOptionA, uiOptionC, dwPad, Func,
                         lpParam, app.GetStringTable(), NULL, 0);
...
void RegisterAwardsWithProfileManager() {
    ...
    // this!\n");
    ProfileManager.RichPresenceInit(
        -1, -1);  // 4J-JEV - Presence and Context count not used.
    ProfileManager.SetRichPresenceSettingFn(
        SQRNetworkManager_PS3::SetRichPresence);
...
int LoadSysModules() {
    ...
    if (ret < 0) {
        ...
        DEBUG_PRINTF(
            "  get_type = [%d] get_attributes = [0x%08x] dirName = [%s]\n",
    ...
    if (uiAttributes & CELL_GAME_ATTRIBUTE_PATCH) {
        ...
        // To access patch game data, obtain its path by separately using
        // cellGamePatchCheck().
    ...
    if (ret < 0) {
        ...
        DEBUG_PRINTF(
            "cellGamePatchCheck() OK - we were booted from a disc patch!\n");
...
int main() {
    ...
    printf("available_user_memory : %.02f",
           mem_info.available_user_memory / (1024.0f * 1024.0f));
    printf("------------------------------------------------------\n");
    printf("------------------------------------------------------\n");
#endif
    ...
    // Sony request that we have enough space for a save game, so people don't
    // get hit with this when they first save. Going to make the space 20MB
...

    {
        ...
        char selfPath[256];
        sprintf(selfPath, "%s%s%s", getUsrDirPath(), SPU_SELF_PATH,
                "mssraw.elf");
        ...
        case e_sku_SCEE:
            ...
            if (StorageManager.GetBootTypeDisc()) {
                ...
                ProfileManager.SetAustraliaMinimumAge(
                    8);  // PG rating has no age, but for some reason the
                         // testers are saying it's 8
    ...
    // Initialise NpBasic - this used to be in the network manager but other
    // things need this so moved out on its own
    ...
    // between, and the number of actions Need to init Input before UI so we
    // know what the Circle/Cross state is
    ...
    ProfileManager.Initialise(
        ...
        SQRNetworkManager_PS3::GetSceNpCommsSig(), PROFILE_VERSION_10,
    ...
    // initialise the storage manager with a callback for displaying the saving
    // message
    StorageManager.Init(
    ...
    // MGH - I've had to rearrange some of the ordering here, as I was getting
    // conflicts with the sysutils for trophies and the upgrade key
    ...
    SonyRemoteStorage_PS3* pRemote =
        (SonyRemoteStorage_PS3*)app.getRemoteStorage();
    ...
    StorageManager.InitialiseProfileData(
        PROFILE_VERSION_10, NUM_PROFILE_VALUES, NUM_PROFILE_SETTINGS,
        dwProfileSettingsA,
        app.GAME_DEFINED_PROFILE_DATA_BYTES * XUSER_MAX_COUNT,
        &app.uiGameDefinedDataChangedBitmask);
    ...
    // 4J-PB - TRC that we need to be able to see the BD in a different titled
    // save from the digital game
    if (StorageManager.GetBootTypeDisc()) {
        ...
        StorageManager.SetGameSaveFolderTitle((WCHAR*)app.GetString(
            IDS_GAMENAME));  //"Minecraft: PlayStation®3 Edition");//GAMENAME);
    }
    StorageManager.SetSaveCacheFolderTitle((WCHAR*)app.GetString(
        IDS_SAVECACHEFILE));  //"Minecraft: PlayStation®3 Edition");//GAMENAME);
    StorageManager.SetOptionsFolderTitle((WCHAR*)app.GetString(
        IDS_OPTIONSFILE));  //"Minecraft: PlayStation®3 Edition");//GAMENAME);
    ...
    if (baSaveLoadIcon.data != NULL) {
        StorageManager.SetSaveLoadIcon((PBYTE)baSaveLoadIcon.data,
                                       baSaveLoadIcon.length);
    ...
    // table loaded
    ProfileManager.SetTrialTextStringTable(NULL, IDS_CONFIRM_OK,
                                           IDS_CONFIRM_CANCEL);
    ...
    InputManager.SetLostInputControlCallback(&UIController::SetSystemUIShowing,
                                             &ui);
...
#pragma message(__LOC__ "Need to define the _FINAL_BUILD before submission")
    ...
    // LeaderboardManager::Instance()->ReadStats_TopRank(DebugReadListener::m_instance,
    // 0, LeaderboardManager::eStatsType_Travelling, 1, 1);

    // 4J-PB - we really need to know at this point if we are playing the trial
    // or full game, so sleep until we know
    while (app.GetCommerce()->LicenseChecked() == false) {
        cellSysutilCheckCallback();
        Sleep(50);
    }
    // wait for the trophy init to complete - nonblocking semaphore
    while ((ProfileManager.tryWaitTrophyInitComplete() < 0) &&
    ...
    // 4J-PB - get the chat/UGC restriction flag here so we can stop online
    // games, and content rating and restriction It's ok to do this for the
    // primary PSN player here - it has this data locally. All other players
    // need to do it on PSN sign in.

    // 	bool bChatRestricted=false;
    // 	ProfileManager.GetChatAndContentRestrictions(0,&bChatRestricted,NULL,NULL);

    // 4J-PB - really want to wait until we've read the options, so we can set
    // the right language if they've chosen one other than the default
    // 	bool bOptionsRead=false;
    //
    // 	while((bOptionsRead==false) &&
    // ShutdownManager::ShouldRun(ShutdownManager::eMainThread))
    // 	{
    ...
    while (ShutdownManager::ShouldRun(ShutdownManager::eMainThread)) {
        ...
        PIXEndNamedEvent();
        // 4J-PB - these get set every tick causing the write profile flag to be
        // true all the time -
        ...
        if (app.GetGameStarted()) {
            // 			if(InputManager.ButtonPressed(0,
